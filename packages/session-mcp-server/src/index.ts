 ...
 * It uses the shared handlers from @crabpal/session-tools-core to ensure
 ...
 *   --workspace-root: Path to workspace folder (~/.crabpal/workspaces/{id})
...
import {
  ...
  errorResponse,
} from '@crabpal/session-tools-core';
...
function createCodexContext(config: SessionConfig): SessionToolContext {
  ...
  return {
    ...
    updatePreferences: (updates: Record<string, unknown>) => {
      ...
      // workspaceRootPath = ~/.crabpal/workspaces/{id}
      // preferencesPath = ~/.crabpal/preferences.json
    ...
    submitFeedback: (feedback) => {
      const configDir = process.env.CRAB_PAL_CONFIG_DIR || join(workspaceRootPath, '..', '..');
...
/**
 * Connect to the built-in docs MCP server (crabpal-docs) and fetch its tool definitions.
...
async function connectDocsUpstream(): Promise<void> {
  try {
    ...
    console.error(`CrabPal Docs proxy connected: ${docsTools.length} tools`);
  } catch (err) {
    console.error(`CrabPal Docs proxy connection failed (non-fatal): ${err instanceof Error ? err.message : String(err)}`);
    docsClient = null;
    docsTools = [];
  }
}
...
async function callDocsUpstream(
  ...
  if (!docsClient) {
    return errorResponse(`CrabPal Docs server is not connected. Tool '${name}' unavailable.`);
  }
...
async function handleCallLlm(
  ...
  // Uses callbackPort from CLI arg (--callback-port) or env var (CRAB_PAL_LLM_CALLBACK_PORT).
  ...
  return errorResponse(
    ...
    'HTTP callback (CRAB_PAL_LLM_CALLBACK_PORT). Neither is available.'
  );
}
...
async function main() {
  ...
  const config: SessionConfig = {
    ...
    callbackPort: callbackPort || process.env.CRAB_PAL_LLM_CALLBACK_PORT,
  };
