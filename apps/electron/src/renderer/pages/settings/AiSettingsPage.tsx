...
import { Spinner, FullscreenOverlayBase } from '@crabpal/ui'
import { useSetAtom } from 'jotai'
import { fullscreenOverlayOpenAtom } from '@/atoms/overlay'
import { motion, AnimatePresence } from 'motion/react'
import type { LlmConnectionWithStatus, ThinkingLevel, WorkspaceSettings, Workspace } from '../../../shared/types'
import { DEFAULT_THINKING_LEVEL, THINKING_LEVELS } from '@crabpal/shared/agent/thinking-levels'
    ...
    switch (provider) {
      ...
      case 'pi': {
        ...
        const piLabel = !isSubscription && connection.piAuthProvider
          ...
          : null
        parts.push(piLabel ?? 'CrabPal Backend')
        break
      }
      case 'pi_compat': parts.push('CrabPal Backend Compatible'); break
                ...
                options={[
                  ...
                  ...llmConnections.map((conn) => ({
                    ...
                    description: conn.providerType === 'anthropic' ? 'Anthropic' :
                                 conn.providerType === 'pi' ? 'CrabPal Backend' :
          ...
          <div className="px-5 py-7 max-w-3xl mx-auto">
            ...
            <div className="space-y-8">
                  ...
                  <SettingsMenuSelectRow
                    ...
                    options={llmConnections.map((conn) => ({
                      ...
                      description: conn.providerType === 'anthropic' ? 'Anthropic API' :
                                   conn.providerType === 'pi' ? 'CrabPal Backend' :
                                   conn.providerType === 'pi_compat' ? 'CrabPal Backend Compatible' :
                                   conn.providerType || 'Unknown',
                    }))}
                  />
                  <SettingsMenuSelectRow
              ...
              <FullscreenOverlayBase
                ...
                className="z-splash flex flex-col bg-[radial-gradient(circle_at_top_left,rgba(13,148,136,0.12),transparent_32%),radial-gradient(circle_at_top_right,rgba(245,158,11,0.1),transparent_24%),linear-gradient(180deg,color-mix(in_oklch,var(--background)_96%,white_4%)_0%,color-mix(in_oklch,var(--background)_92%,black_8%)_100%)]"
              >
                <OnboardingWizard
                ...
                />
                <div
                  className="fixed top-0 right-0 h-[50px] flex items-center pr-5 [-webkit-app-region:no-drag] z-splash"
                >
                  <button
