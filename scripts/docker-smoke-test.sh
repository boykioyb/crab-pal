#!/usr/bin/env bash
# =============================================================================
# Docker smoke test for CrabPal Server
#
...
while [ "$ELAPSED" -lt "$TIMEOUT" ]; do
  ...
  if docker logs "$CONTAINER_NAME" 2>&1 | grep -q "CRAB_PAL_SERVER_URL="; then
    READY=true
    break
  fi
