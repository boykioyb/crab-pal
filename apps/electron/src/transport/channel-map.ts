...
export const CHANNEL_MAP = {
  ...
  legacyImportRun: invoke(RPC_CHANNELS.legacyImport.RUN),
  onLegacyImportProgress: listener(RPC_CHANNELS.legacyImport.PROGRESS),

  // Logs viewer
  readLogs: invoke(RPC_CHANNELS.logs.READ),
  getLogsFilePath: invoke(RPC_CHANNELS.logs.GET_FILE_PATH),
  startLogsTail: invoke(RPC_CHANNELS.logs.TAIL_START),
  stopLogsTail: invoke(RPC_CHANNELS.logs.TAIL_STOP),
  onLogsNewLines: listener(RPC_CHANNELS.logs.NEW_LINES),
  logRenderer: invoke(RPC_CHANNELS.logs.RENDERER),

  // Projects (Workspace → Project → Session hierarchy)
  'projects.list': invoke(RPC_CHANNELS.projects.LIST),
  'projects.get': invoke(RPC_CHANNELS.projects.GET),
  'projects.create': invoke(RPC_CHANNELS.projects.CREATE),
  'projects.update': invoke(RPC_CHANNELS.projects.UPDATE),
