...
import { getSessionSafeAllowedToolNames } from '@crabpal/session-tools-core';
...
class ModeManager {
  ...
  setPermissionMode(
    ...
    // Notify callbacks (for ClaudeAgent internal sync)
...
export function getPathHint(targetPath: string, plansFolderPath: string, dataFolderPath?: string): string | null {
  ...
  if (normalizedTarget.includes('/.crabpal/workspaces/') && !normalizedTarget.includes('/sessions/')) {
    return 'Hint: Write to the session plans or data folder, not the workspace root.';
  }

  // Case: Writing outside .crabpal entirely
  if (!normalizedTarget.includes('/.crabpal/')) {
...
export function shouldAllowToolInMode(
  ...
  if (toolName.startsWith('mcp__')) {
    ...
    if (toolName.startsWith('mcp__crabpal-docs__')) {
      return { allowed: true };
    }
