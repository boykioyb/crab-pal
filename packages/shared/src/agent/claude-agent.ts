...
// Plan types are used by UI components; not needed in crabpal.ts since Safe Mode is user-controlled
...
// Documentation is served via local files at ~/.crabpal/docs/
...
export class ClaudeAgent extends BaseAgent {
  ...
  /** Rate limit info by type, accumulated across SDK rate_limit_event messages */
  private rateLimitInfoByType: Map<string, SDKRateLimitInfo> = new Map();
  /** Cached account info from last successful accountInfo() call */
  private cachedAccountInfo: import('@anthropic-ai/claude-agent-sdk').AccountInfo | null = null;
  ...
  override async postInit(): Promise<PostInitResult> {
    ...
    // Write auth vars into per-session envOverrides — NOT process.env.
    // buildClaudeSubprocessEnv() clears auth keys before merging envOverrides,
    // so each subprocess gets only this session's credentials regardless of what
    // other concurrent sessions wrote (or are writing) to process.env.
    this.config.envOverrides = {
      ...this.config.envOverrides,
      ...result.envVars,
      ...(this.config.miniModel
        ? { ANTHROPIC_DEFAULT_HAIKU_MODEL: this.config.miniModel }
        : {}),
    };
...
This is a branched conversation. All prior messages in this conversation are part of your shared context with the user. When the user refers to "this conversation" or asks what you discussed, include the full conversation history — not just messages after the branch point.
      ...
      // Eagerly fetch and cache account info on first turn (non-blocking)
      if (!this.cachedAccountInfo && this.currentQuery) {
        const q = this.currentQuery;
        q.accountInfo().then((info) => { if (info) this.cachedAccountInfo = info; }).catch(() => {});
      }
      ...
      try {
        for await (const message of this.currentQuery) {
          ...
          // Capture rate limit info for account/usage display (accumulate by type)
          if ('type' in message && message.type === 'rate_limit_event' && 'rate_limit_info' in message) {
            const rl = (message as { rate_limit_info: SDKRateLimitInfo }).rate_limit_info;
            this.rateLimitInfoByType.set(rl.rateLimitType ?? 'unknown', rl);
          }
  ...
  /**
   * Get all accumulated rate limit info captured from SDK events.
   * Returns an empty array if no rate limit events have been received yet.
   */
  getRateLimitInfo(): SDKRateLimitInfo[] {
    return Array.from(this.rateLimitInfoByType.values());
  }

  /**
   * Get account info from the active SDK query.
   * Returns null if no active query is available.
   */
  async getAccountInfo(): Promise<import('@anthropic-ai/claude-agent-sdk').AccountInfo | null> {
    if (this.currentQuery) {
      try {
        const info = await this.currentQuery.accountInfo();
        if (info) this.cachedAccountInfo = info;
        return info;
      } catch {
