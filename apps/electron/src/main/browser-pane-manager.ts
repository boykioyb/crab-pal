...
import { DEFAULT_THEME, loadAppTheme } from '@crabpal/shared/config'
import { getBrowserLiveFxCornerRadii } from '../shared/browser-live-fx'
import type { IBrowserPaneManager } from '@crabpal/server-core/handlers'
...
const THEME_COLOR_SIGNAL_PREFIX = '__crabpal_theme_color__:'
...
const CRAB_PAL_DEEPLINK_SCHEME_PREFIX = `${process.env.CRAB_PAL_DEEPLINK_SCHEME || 'crabpal'}://`
...
export class BrowserPaneManager implements IBrowserPaneManager {
  ...
  private buildDeepLinkFromRoute(route: string): string {
    ...
    return `${CRAB_PAL_DEEPLINK_SCHEME_PREFIX}${normalizedPath}${routeQuery ? `?${routeQuery}` : ''}`
  }
...
<html>
  ...
  private async handleDeepLinkUrl(url: string): Promise<void> {
    if (!url.startsWith(CRAB_PAL_DEEPLINK_SCHEME_PREFIX)) return
...
<html>
  ...
  private installThemeObserver(instance: BrowserInstance, allowRetry = true): void {
    ...
    void instance.pageView.webContents.executeJavaScript(`
      (() => {
        ...
        const w = window;
        const previousCleanup = w.__CRAB_PAL_THEME_OBSERVER_CLEANUP__;
        ...
        w.__CRAB_PAL_THEME_OBSERVER_CLEANUP__ = () => {
  ...
  private setupWindowListeners(instance: BrowserInstance): void {
    ...
    pageWc.on('will-navigate', (event, url) => {
      if (url.startsWith(CRAB_PAL_DEEPLINK_SCHEME_PREFIX)) {
        event.preventDefault()
    ...
    pageWc.setWindowOpenHandler((details) => {
      ...
      if (details.url.startsWith(CRAB_PAL_DEEPLINK_SCHEME_PREFIX)) {
