 ...
 * Extracted here so handler code in @crabpal/server-core can import
 * from @crabpal/shared/protocol without reaching into the app.
 */
...
export { generateMessageId } from '@crabpal/core/types'
...
export interface Session {
   ...
   *  Optional only for backwards-compatibility with pre-project sessions;
   *  new sessions created via createSession() always receive a projectId. */
  projectId?: string
  name?: string
...
export interface CreateSessionOptions {
  /** Project (inside the workspace) the new session should belong to.
   *  Required as of Phase 3. SessionManager.createSession rejects payloads
   *  missing this field — callers must resolve a project (e.g. the active
   *  project atom in the renderer) before invoking createSession. */
  projectId: string
  name?: string
  permissionMode?: PermissionMode
  /**
