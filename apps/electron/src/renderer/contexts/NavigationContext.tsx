...
import { parsePermissionMode } from '@crabpal/shared/agent/mode-types'
...
export function NavigationProvider({
  ...
  const handleActionNavigation = useCallback(
    async (parsed: ParsedRoute, options?: { newPanel?: boolean; targetLaneId?: 'main' }) => {
      ...
      switch (parsed.name) {
        case 'new-session': {
          // Resolve projectId: active project atom, else first project in the workspace.
          // If no projects exist, surface a clear error — the user must create one first.
          const activeProjectId = store.get(activeProjectIdAtom)
          ...
          if (!resolvedProjectId) {
            toast.error('No project available', {
              description: 'Create a project before starting a new session.',
            })
            return
          }
