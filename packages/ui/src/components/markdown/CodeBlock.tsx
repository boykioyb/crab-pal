...
export function CodeBlock({ code, language = 'text', className, mode = 'full', forcedTheme }: CodeBlockProps) {
  ...
  return (
    <div className={cn('relative group rounded-lg overflow-hidden border border-border/30 bg-background/60 backdrop-blur-sm', className)}>
      {/* Language label + copy button */}
      <div className="flex items-center justify-between px-3 py-1.5 bg-foreground/[0.03] border-b border-border/30 text-xs">
        <span className="text-muted-foreground/80 font-medium uppercase tracking-wide">
