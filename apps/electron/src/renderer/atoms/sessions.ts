...
export interface SessionMeta {
   ...
   *  Required for sessions created after Phase 3; legacy sessions on disk may
   *  surface without a projectId and should be treated as unassigned. */
  projectId?: string
...
/**
 * Derived: sessions grouped by projectId.
 *
 * Useful for the sidebar's by-project grouping mode (Phase 4). Sessions
 * without a projectId (legacy pre-Phase-3 sessions) are bucketed under the
 * `UNASSIGNED_PROJECT_BUCKET` key so the UI can decide how to render them.
 */
export const UNASSIGNED_PROJECT_BUCKET = '__unassigned__' as const

export const sessionsByProjectAtom = atom<Map<string, SessionMeta[]>>((get) => {
  const metaMap = get(sessionMetaMapAtom)
  const grouped = new Map<string, SessionMeta[]>()
  for (const meta of metaMap.values()) {
    const key = meta.projectId ?? UNASSIGNED_PROJECT_BUCKET
  ...
  // Sort each bucket by lastMessageAt desc so callers get a stable order.
...
/**
 * Action atom: create a session via IPC under a specific project.
 *
 * Phase 3 requires every session to have a projectId at creation time.
 * Call sites that previously invoked `window.electronAPI.createSession`
 * directly should route through this atom so the invariant is enforced
 * uniformly and the new session is registered with per-session atoms.
 */
export const createSessionAtom = atom(
  ...
  async (
    ...
    args: {
      ...
      projectId: string
      options?: Omit<
        import('../../shared/types').CreateSessionOptions,
        'projectId'
      >
    },
  ): Promise<import('../../shared/types').Session> => {
    if (!args.projectId) {
      throw new Error('createSessionAtom: projectId is required')
    }
    const session = await window.electronAPI.createSession(args.workspaceId, {
      ...args.options,
      projectId: args.projectId,
    })
    // Register with per-session atoms + metadata map.
    set(addSessionAtom, session)
    return session
  },
)
