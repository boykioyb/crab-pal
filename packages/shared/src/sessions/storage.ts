...
export async function createSession(
  ...
  options?: {
     ...
     *  Required for sessions created after the Workspace → Project → Session
     *  hierarchy was introduced. Optional here only so legacy/edge call sites
     *  can keep compiling; SessionManager.createSession enforces the invariant. */
    projectId?: string;
...
/**
 * Backfill missing projectId on legacy sessions by assigning them to the
 * provided default project. Pre-hierarchy sessions have no projectId and
 * otherwise bucket as "Unassigned" in the sidebar.
 *
 * Returns the number of sessions patched.
 */
export async function backfillProjectIds(
  ...
  for (const meta of sessions) {
    ...
    await updateSessionMetadata(workspaceRootPath, meta.id, { projectId: defaultProjectId });
    patched += 1;
  }
  return patched;
}
