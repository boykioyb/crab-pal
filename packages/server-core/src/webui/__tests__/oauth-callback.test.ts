...
function createTestWebuiDir(): string {
  const dir = mkdtempSync(join(tmpdir(), 'crabpal-webui-oauth-test-'));
  TEMP_DIRS.push(dir);
...
describe('WebUI /api/oauth/callback', () => {
  it('completes OAuth when the relay forwards the inner flow state', async () => {
    const flow = {
      ...
      redirectUri: 'https://crabpal.app/auth/callback',
      source: {} as any,
    ...
    handler.setOAuthCallbackDeps({
      ...
      credManager: {
        exchangeAndStore: async () => ({ success: true, email: 'gyula@crabpal.app' }),
      },
      sessionManager: {
