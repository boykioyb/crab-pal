...
interface ChatDisplayProps {
  ...
  /** Handler for error recovery actions (Settings, Retry, Reauth buttons on error messages) */
  onErrorAction?: (action: 'retry' | 'settings' | 'reauth' | 'reconnect_source', sourceSlug?: string) => void
  // Model selection
  currentModel: string
  ...
  labels?: import('@crabpal/shared/labels').LabelConfig[]
...
export const ChatDisplay = React.forwardRef<ChatDisplayHandle, ChatDisplayProps>(function ChatDisplay({
  ...
  const prevFocusedPanelRef = React.useRef(isFocusedPanel)
  const prevIsFocusedRef = React.useRef(isFocused)
  const prevSessionForFocusRef = React.useRef(session?.id ?? null)
  useEffect(() => {
    const prevFocusedPanel = prevFocusedPanelRef.current
    const prevIsFocused = prevIsFocusedRef.current
    const prevSessionId = prevSessionForFocusRef.current
    prevFocusedPanelRef.current = isFocusedPanel
    prevIsFocusedRef.current = isFocused
    prevSessionForFocusRef.current = session?.id ?? null

    // Only focus on the transition into a focused state (or on session switch),
    // not on every rerender where isFocused/isFocusedPanel happen to be true.
    const becameFocusedPanel = isFocusedPanel && !prevFocusedPanel
    const becameFocused = isFocused && !prevIsFocused
    const sessionSwitched = (session?.id ?? null) !== prevSessionId
    if (!becameFocusedPanel && !becameFocused && !sessionSwitched) return
  ...
  // These settings are stored in ~/.crabpal/preferences.json (not localStorage)
  ...
  React.useEffect(() => {
    ...
    // Pause auto-scroll briefly after any user-initiated scroll so that
    // ResizeObserver firings triggered by scroll-induced layout shifts
    // (revealed content, sticky headers, etc.) don't fight the user.
    ...
    const resizeObserver = new ResizeObserver(() => {
      const withinUserScroll = Date.now() - lastUserScrollAt < USER_SCROLL_COOLDOWN_MS

      // Unfocused panels: always scroll to bottom instantly (user isn't reading them)
      if (!isFocusedPanelRef.current) {
        if (withinUserScroll) return
      ...
      debounceTimer = setTimeout(() => {
        ...
        if (Date.now() - lastUserScrollAt < USER_SCROLL_COOLDOWN_MS) return
  ...
  const handleSaveAndSendFollowUp = useCallback((_target: {
    ...
    window.setTimeout(() => {
      window.dispatchEvent(new CustomEvent('crabpal:submit-input', {
      ...
      {session ? (
        <div className="flex flex-1 flex-col min-h-0 min-w-0 relative">
          ...
          <div className="relative flex-1 min-h-0">
            {/* Scroll container with gradient fade overlays at top/bottom.
             * Previously used `mask-image` which forced the browser to re-rasterize the
             * masked region on every scroll frame, bleeding repaints into the vibrancy/
             * backdrop-filter layers of adjacent panels (sidebar flicker). Replaced with
             * absolutely-positioned gradient overlays — GPU blend only, no mask. */}
            <div className="h-full relative">
              ...
              <div className={cn(
                ...
                <AnimatePresence mode={compactMode ? "sync" : "wait"} initial={false}>
                  ...
                  {hasMoreAbove && (
                    <div className="text-center text-muted-foreground/50 text-xs py-3 select-none tracking-[-0.01em]">
                    ...
                    return (
                      ...
                      <TurnCard
                          ...
                          try {
                            if (!session.projectId) {
                              toast.error('Could not create branch', {
                                description: 'Parent session is missing a project. Open it once to assign a project, then try again.',
                              })
                              return
                            }
                        ...
                        onAcceptPlan={() => {
                          ...
                          window.dispatchEvent(new CustomEvent('crabpal:approve-plan', {
                            detail: {
                              sessionId: session?.id,
                              planPath,
                        ...
                        onAcceptPlanWithCompact={() => {
                          ...
                          window.dispatchEvent(new CustomEvent('crabpal:approve-plan-with-compact', {
                            detail: {
                              sessionId: session?.id,
                              planPath,
                              includeDraftInput: true,
              ...
              {/* Top/bottom fade overlays (replacement for mask-image). Pointer-events disabled
               * so the user can still scroll through them. Uses --background so it adapts to theme. */}
              <div
...
interface MessageBubbleProps {
  ...
  onOpenFile: (path: string) => void
  onOpenUrl: (url: string) => void
  /** Handler for error recovery actions on error messages */
  onErrorAction?: (action: 'retry' | 'settings' | 'reauth' | 'reconnect_source', sourceSlug?: string) => void
  /**
...
function ErrorMessage({
  ...
  const actions = message.errorActions?.filter(a => {
    ...
    if (a.action === 'retry' || a.action === 'settings' || a.action === 'reauth' || a.action === 'reconnect_source') {
            ...
            {actions.map((action) => (
              <button
                ...
                onClick={() => {
                  ...
                  if (action.action && onErrorAction) {
                    onErrorAction(
                      action.action as 'retry' | 'settings' | 'reauth' | 'reconnect_source',
                      action.sourceSlug,
                    )
                  }
                }}
                className="text-xs px-2 py-0.5 rounded-md border border-destructive/20 text-destructive/70 hover:text-destructive hover:border-destructive/40 transition-colors duration-150"
              >
...
function MessageBubble({
  ...
  if (message.role === 'assistant') {
    return (
      <div className="flex justify-start group">
        <div className="relative max-w-[90%] bg-background/80 backdrop-blur-sm border border-border/30 shadow-sm rounded-lg pl-6 pr-4 py-3 break-words min-w-0 select-text">
          {/* Pop-out button - visible on hover */}
          {onPopOut && !message.isStreaming && (
            <button
              onClick={() => onPopOut(message)}
              className="absolute top-2 right-2 p-1.5 rounded-md opacity-0 group-hover:opacity-100 transition-opacity duration-150 hover:bg-foreground/5"
  ...
  if (message.role === 'error') {
    return <ErrorMessage message={message} onOpenUrl={onOpenUrl} onErrorAction={onErrorAction} />
  }
    ...
    if (message.statusType === 'compaction_complete') {
      return (
        <div className="flex items-center gap-3 my-12 px-3">
          ...
          <span className="text-xs font-medium text-muted-foreground/60 select-none tracking-[-0.01em]">
  ...
  if (message.role === 'warning') {
    return (
      <div className="flex justify-start">
        <div className="max-w-[80%] bg-info/10 border border-info/20 rounded-lg pl-5 pr-4 pt-2 pb-2.5 break-words select-none">
...
const MemoizedMessageBubble = React.memo(MessageBubble, (prev, next) => {
  ...
  return (
    ...
    prev.onErrorAction === next.onErrorAction
  )
})
