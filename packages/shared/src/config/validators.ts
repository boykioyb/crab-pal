  ...
  try {
    ...
    return {
      ...
      errors: [{
        ...
        severity: 'error',
        suggestion: 'See ~/.crabpal/docs/skills.md for SKILL.md format reference',
      }],
      warnings: [],
    };
  }
...
/**
 * Zod schema for app-level theme override files (~/.crabpal/theme.json).
        ...
        if (tool.icon) {
          ...
          if (!existsSync(iconPath)) {
            result.warnings.push({
              ...
              severity: 'warning',
              suggestion: `Place '${tool.icon}' in ~/.crabpal/tool-icons/`,
            });
          }
        }
      }
 ...
 * Checks paths relative to CONFIG_DIR (~/.crabpal/).
 * Returns null if the path is not a recognized app-level config file.
 *
 * Matches patterns:
 * - ~/.crabpal/tool-icons/tool-icons.json → tool icon mappings
 */
