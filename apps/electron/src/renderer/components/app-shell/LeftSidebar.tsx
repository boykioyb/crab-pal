 ...
 * - isCollapsed=true renders a 48px icon-only vertical rail
 * - Each item shows only its icon, centered, with a title tooltip
 * - Separators render as short horizontal dividers
 * - Active state: text-foreground + bg-foreground/10 rounded-lg pill
 */
export function LeftSidebar({ links, isCollapsed, getItemProps, focusedItemId, isNested }: LeftSidebarProps) {
  // Icon rail mode — render only icons, centered, with tooltips
        ...
        >
          {links.map((item) => {
            if (isSeparatorItem(item)) {
              return (
                <div key={item.id} className="py-1 flex justify-center" aria-hidden="true">
            ...
            return (
              <button
                ...
                className={cn(
                  "w-full flex items-center justify-center h-9 rounded-lg my-0.5 transition-colors duration-150 outline-none",
  ...
  ({ link, itemProps, isOverlay, className: extraClassName, ...radixProps }, forwardedRef) => {
    return (
      <button
        ...
        data-tutorial={link.dataTutorial}
        className={cn(
          "group flex w-full items-center gap-2 rounded-lg border border-transparent text-[13px] select-none outline-none transition-colors",
          "focus-visible:ring-1 focus-visible:ring-inset focus-visible:ring-ring",
          // Compact mode: 4px less total height (py-[3px] vs py-[5px])
          link.compact ? "py-[3px]" : "py-[6px]",
          "px-2.5",
          link.variant === "default"
            ? "border-foreground/8 bg-foreground/[0.08] shadow-[0_10px_20px_-18px_rgba(15,23,42,0.55)]"
            // Highlight on hover, context menu open (data-state), or EditPopover active (data-edit-active)
            : "text-foreground/72 hover:border-foreground/6 hover:bg-foreground/[0.045] data-[state=open]:border-foreground/8 data-[state=open]:bg-foreground/[0.06] data-[edit-active=true]:border-foreground/8 data-[edit-active=true]:bg-foreground/[0.06]",
          extraClassName,
        )}
      >
...
/**
 * Renders icon at 18px for the collapsed icon rail.
 * Uses currentColor so the button's text color controls icon color.
 */
function renderIconCollapsed(link: LinkItem) {
  const isComponent = typeof link.icon === 'function' ||
    (typeof link.icon === 'object' && link.icon !== null && 'render' in link.icon)
  ...
  // ReactNode icon (e.g., span with nested svg like "Updates" badge icon)
  const iconElement = link.icon as React.ReactNode
  const bareIcon = React.isValidElement(iconElement)
    ? (typeof iconElement.type === 'function' && (iconElement.type as { acceptsBare?: boolean }).acceptsBare)
      ? React.cloneElement(iconElement as React.ReactElement<{ bare?: boolean }>, { bare: true })
    ...
    : iconElement
  return (
    <span className="h-[18px] w-[18px] shrink-0 flex items-center justify-center [&>svg]:w-full [&>svg]:h-full">
      {bareIcon}
    </span>
  )
}
