...
import { appendAutomationHistoryEntry } from '@crabpal/shared/automations/history-store'
import { AUTOMATION_HISTORY_MAX_RUNS_PER_MATCHER } from '@crabpal/shared/automations/constants'
import type { RpcServer } from '@crabpal/server-core/transport'
import type { HandlerDeps } from '../handler-deps'

// History file name — matches AUTOMATIONS_HISTORY_FILE from @crabpal/shared/automations/constants
...
async function withAutomationMatcher(workspaceId: string, eventName: string, matcherIndex: number, mutate: (matchers: Record<string, unknown>[], index: number, config: AutomationsConfigJson, genId: () => string) => void) {
  ...
  await withConfigMutex(workspace.rootPath, async () => {
    const { resolveAutomationsConfigPath, generateShortId } = await import('@crabpal/shared/automations/resolve-config-path')
...
export function registerAutomationsHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(RPC_CHANNELS.automations.TEST, async (_ctx, payload: import('@crabpal/shared/protocol').TestAutomationPayload) => {
    const workspace = getWorkspaceByNameOrId(payload.workspaceId)
    if (!workspace) throw new Error('Workspace not found')

    const results: import('@crabpal/shared/protocol').TestAutomationActionResult[] = []
    const { parsePromptReferences } = await import('@crabpal/shared/automations')
    const { executeWebhookRequest, createWebhookHistoryEntry, createPromptHistoryEntry } = await import('@crabpal/shared/automations/webhook-utils')
    ...
    for (const action of payload.actions) {
      ...
      if (action.type === 'webhook') {
        ...
        const result = await executeWebhookRequest(action as import('@crabpal/shared/automations').WebhookAction)
    ...
    return { actions: results } satisfies import('@crabpal/shared/protocol').TestAutomationResult
  })
  ...
  server.handle(RPC_CHANNELS.automations.REPLAY, async (_ctx, workspaceId: string, automationId: string, eventName: string) => {
    ...
    const { resolveAutomationsConfigPath } = await import('@crabpal/shared/automations/resolve-config-path')
    const configPath = resolveAutomationsConfigPath(workspace.rootPath)
    const raw = await readFile(configPath, 'utf-8')
    ...
    const { executeWebhookRequest, createWebhookHistoryEntry } = await import('@crabpal/shared/automations/webhook-utils')
    const results = await Promise.all(
      webhookActions.map(a => executeWebhookRequest(a as unknown as import('@crabpal/shared/automations').WebhookAction))
    )
