...
const ChatPage = React.memo(function ChatPage({ sessionId }: ChatPageProps) {
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('crabpal:restore-input', handler)
    return () => window.removeEventListener('crabpal:restore-input', handler)
  }, [sessionId])
  ...
  //
  // `sessionOverrideDir` is the raw per-session override (undefined = use
  // project default). `effectiveWorkingDirectory` collapses override +
  // project.rootPath into the single path the agent / file resolver should
  // actually use (see getEffectiveWorkingDirectory).
  const sessionOverrideDir = session?.workingDirectory
  const activeWorkspace = React.useMemo(
    () => workspaces.find((w) => w.id === activeWorkspaceId) || null,
    [workspaces, activeWorkspaceId]
  )
  const sessionProject = useAtomValue(projectByIdAtom(session?.projectId ?? ''))
  const effectiveWorkingDirectory = React.useMemo(() => {
    if (!session) return undefined
    if (sessionProject) {
      return getEffectiveWorkingDirectory(session, sessionProject)
    }
    return sessionOverrideDir
  }, [session, sessionProject, sessionOverrideDir])
  // `workingDirectory` passed to <ChatDisplay>/<FreeFormInput> represents the
  // *raw* session-level override so the ProjectBadge can distinguish overridden
  // vs. default. File-resolution / other effective-cwd uses should reference
  // `effectiveWorkingDirectory` instead.
  const workingDirectory = sessionOverrideDir
  ...
  const handleOpenFile = React.useCallback(
    async (path: string) => {
      ...
      const resolved = (() => {
        ...
        const baseDir = effectiveWorkingDirectory || activeWorkspace?.rootPath
    ...
    [onOpenFile, effectiveWorkingDirectory, activeWorkspace?.rootPath]
  )
  ...
  const handleOpenInNewWindow = React.useCallback(async () => {
    ...
    const url = `crabpal://${route}${separator}window=focused`
    try {
      ...
      <StyledDropdownMenuContent align="end" sideOffset={8}>
        {sharedUrl ? (
          <>
            ...
            <StyledDropdownMenuSeparator />
            <StyledDropdownMenuItem onClick={() => window.electronAPI.openUrl('https://crabpal.app/docs/go-further/sharing')}>
  ...
  // Handler for error message action buttons (Settings, Retry, Reauth, Reconnect)
  const handleErrorAction = React.useCallback(
    (action: 'retry' | 'settings' | 'reauth' | 'reconnect_source', _sourceSlug?: string) => {
      switch (action) {
        case 'settings':
          onOpenSettings()
          break
        case 'retry': {
          ...
          if (lastUserMessage && lastUserMessage.content) {
            onSendMessage(session.id, lastUserMessage.content)
          }
          break
        }
        case 'reauth':
    ...
    [session, onSendMessage, onOpenSettings],
  )
