...
// =============================================================================
export * from '@crabpal/shared/protocol'
...
export { PERMISSION_MODE_CONFIG } from '@crabpal/shared/agent/modes';

// Thinking level types
import type { ThinkingLevel } from '@crabpal/shared/agent/thinking-levels';
export type { ThinkingLevel };
export { THINKING_LEVELS, DEFAULT_THINKING_LEVEL } from '@crabpal/shared/agent/thinking-levels';
...
import type { AuthState, SetupNeeds } from '@crabpal/shared/auth/types';
import type { AuthType } from '@crabpal/shared/config/types';
export type { AuthState, SetupNeeds, AuthType };

// Credential health types
import type { CredentialHealthStatus, CredentialHealthIssue, CredentialHealthIssueType } from '@crabpal/shared/credentials/types';
...
import type { LoadedSource, FolderSourceConfig, SourceConnectionStatus } from '@crabpal/shared/sources/types';
export type { LoadedSource, FolderSourceConfig, SourceConnectionStatus };

// Skill types
import type { LoadedSkill, SkillMetadata } from '@crabpal/shared/skills/types';
export type { LoadedSkill, SkillMetadata };

// LLM connection types
import type { LlmConnection, LlmConnectionWithStatus, LlmAuthType, LlmProviderType, NetworkProxySettings } from '@crabpal/shared/config';
...
import type { WorkspaceInfo, Workspace, SessionMetadata, StoredAttachment as StoredAttachmentType } from '@crabpal/core/types';
...
export interface ElectronAPI {
  ...
  getServerConfig(): Promise<import('@crabpal/shared/config/server-config').ServerConfig>
  setServerConfig(config: import('@crabpal/shared/config/server-config').ServerConfig): Promise<void>
  getServerStatus(): Promise<import('@crabpal/shared/config/server-config').ServerStatus>
  ...
  // Project management (Workspace → Project → Session hierarchy)
  projects: {
    list(workspaceId: string): Promise<import('@crabpal/core/types').Project[]>
    get(workspaceId: string, projectId: string): Promise<import('@crabpal/core/types').Project | null>
    create(input: import('@crabpal/core/types').CreateProjectInput): Promise<import('@crabpal/core/types').Project>
    update(
      workspaceId: string,
      projectId: string,
      patch: import('@crabpal/core/types').UpdateProjectPatch,
    ): Promise<import('@crabpal/core/types').Project | null>
    delete(workspaceId: string, projectId: string): Promise<boolean>
  }
  /** Fired when the project list for a workspace changes (add/update/delete). */
  ...
  // Deep link navigation listener (for external crabpal:// URLs)
  ...
  getSourcePermissionsConfig(workspaceId: string, sourceSlug: string): Promise<import('@crabpal/shared/agent').PermissionsConfigFile | null>
  getWorkspacePermissionsConfig(workspaceId: string): Promise<import('@crabpal/shared/agent').PermissionsConfigFile | null>
  getDefaultPermissionsConfig(): Promise<{ config: import('@crabpal/shared/agent').PermissionsConfigFile | null; path: string }>
  ...
  listStatuses(workspaceId: string): Promise<import('@crabpal/shared/statuses').StatusConfig[]>
  ...
  listLabels(workspaceId: string): Promise<import('@crabpal/shared/labels').LabelConfig[]>
  createLabel(workspaceId: string, input: import('@crabpal/shared/labels').CreateLabelInput): Promise<import('@crabpal/shared/labels').LabelConfig>
  ...
  listViews(workspaceId: string): Promise<import('@crabpal/shared/views').ViewConfig[]>
  saveViews(workspaceId: string, views: import('@crabpal/shared/views').ViewConfig[]): Promise<void>
  ...
  // Account info
  getAccountInfo(opts?: { connectionSlug?: string }): Promise<{
    email?: string
    organization?: string
  ...
  getRateLimitInfo(opts?: { force?: boolean; connectionSlug?: string }): Promise<Array<{
  ...
  legacyImportPreview(
    categories: import('@crabpal/shared/config').LegacyCategory[],
  ): Promise<import('@crabpal/shared/config').PreviewResult>
  legacyImportRun(
    options: Omit<import('@crabpal/shared/config').ImportOptions, 'onProgress'>,
  ): Promise<import('@crabpal/shared/config').ImportResult>
  onLegacyImportProgress(
    callback: (event: import('@crabpal/shared/config').ImportProgressEvent) => void,
  ): () => void

  // Log viewer
  readLogs(opts?: { tail?: number }): Promise<{ path: string | null; lines: string[] }>
  getLogsFilePath(): Promise<string | null>
  startLogsTail(): Promise<{ started: boolean }>
  stopLogsTail(): Promise<void>
  onLogsNewLines(callback: (lines: string[]) => void): () => void
  logRenderer(entry: { level: 'info' | 'warn' | 'error'; scope: string; args: unknown[] }): Promise<void>
}
