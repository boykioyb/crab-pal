...
import { bootstrapServer } from '@crabpal/server-core/bootstrap'
import { initModelRefreshService, getModelRefreshService, setFetcherPlatform } from '@crabpal/server-core/model-fetchers'
import { setSearchPlatform, setImageProcessor } from '@crabpal/server-core/services'
import { createApplicationMenu } from './menu'
import { WindowManager } from './window-manager'
import { loadWindowState, saveWindowState } from './window-state'
import { getWorkspaces, getWorkspaceByNameOrId, loadStoredConfig, addWorkspace, saveConfig } from '@crabpal/shared/config'
import { getDefaultWorkspacesDir } from '@crabpal/shared/workspaces'
import { initializeDocs } from '@crabpal/shared/docs'
import { initializeReleaseNotes } from '@crabpal/shared/release-notes'
import { ensureDefaultPermissions } from '@crabpal/shared/agent/permissions-config'
import { ensureToolIcons, ensurePresetThemes } from '@crabpal/shared/config'
import { setBundledAssetsRoot } from '@crabpal/shared/utils'
import { initializeBackendHostRuntime } from '@crabpal/shared/agent/backend'
import { setPowerShellValidatorRoot } from '@crabpal/shared/agent'
import { handleDeepLink } from './deep-link'
import { BrowserPaneManager } from './browser-pane-manager'
import { OAuthFlowStore } from '@crabpal/shared/auth'
import { registerThumbnailScheme, registerThumbnailHandler } from './thumbnail-protocol'
import log, { isDebugMode, mainLog, getLogFilePath } from './logger'
import { setPerfEnabled, enableDebug } from '@crabpal/shared/utils'
import { registerPiModelResolver } from '@crabpal/shared/config'
import { getPiModelsForAuthProvider, getAllPiModels } from '@crabpal/shared/config'
...
import { validateGitBashPath, checkVCRedistInstalled } from '@crabpal/server-core/services'
...
// Custom URL scheme for deeplinks (e.g., crabpal://auth-complete)
...
app.whenReady().then(async () => {
  ...
  // Seed tool icons to ~/.crabpal/tool-icons/ (copies bundled SVGs on first run)
  ensureToolIcons()

  // Seed preset themes to ~/.crabpal/themes/ (copies bundled theme JSONs on first run)
  ensurePresetThemes()
  ...
  try {
    ...
    if (!isClientOnly) {
      ...
      if (process.platform === 'win32') {
        const { getGitBashPath, clearGitBashPath } = await import('@crabpal/shared/config')
      ...
      const { getServerConfig } = await import('@crabpal/shared/config')
      ...
      let tls: import('@crabpal/server-core/transport').WsRpcTlsOptions | undefined
      ...
      // Seed a default Project for every workspace that has none.
      // This makes the Workspace → Project → Session hierarchy valid before
      // the renderer ever asks for projects. Safe no-op when a project exists.
      try {
        const { seedDefaultProjectsOnBoot } = await import('./handlers/index')
        seedDefaultProjectsOnBoot({
          sessionManager: instance.sessionManager,
          platform,
          windowManager: windowManager ?? undefined,
          browserPaneManager: browserPaneManager ?? undefined,
        ...
        console.warn('[projects] seed on boot failed:', err)
      }
      ...
      ipcMain.handle('workspace:remove', async (_event, workspaceId: string) => {
        const { removeWorkspace: remove } = await import('@crabpal/shared/config')
      ...
      instance.wsServer.handle(RPC_CHANNELS.settings.GET_SERVER_CONFIG, async () => {
        const { getServerConfig: getConfig } = await import('@crabpal/shared/config')
        return getConfig()
      })

      instance.wsServer.handle(RPC_CHANNELS.settings.SET_SERVER_CONFIG, async (_ctx: unknown, config: unknown) => {
        const { setServerConfig: setConfig } = await import('@crabpal/shared/config')
        const cfg = config as import('@crabpal/shared/config/server-config').ServerConfig
        // Validate port range
    ...
    try {
      const { getLlmConnection, getDefaultLlmConnection } = await import('@crabpal/shared/config')
