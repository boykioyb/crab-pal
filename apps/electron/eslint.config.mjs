...
export default [
  ...
  // TypeScript/React files
  {
    ...
    plugins: {
      ...
      // Custom plugin for CrabPal rules
      'crabpal': {
        rules: {
    ...
    rules: {
      ...
      // Custom CrabPal rules
      'crabpal/no-direct-navigation-state': 'error',
      'crabpal/no-localstorage': 'warn',

      // Custom platform detection rule
      'crabpal-platform/no-direct-platform-check': 'error',

      // Custom cross-platform path rule
      'crabpal-paths/no-hardcoded-path-separator': 'warn',

      // Custom link interceptor rule — prevents bypassing in-app file preview
      'crabpal-links/no-direct-file-open': 'error',

      // Custom source auth check rule — use isSourceUsable() instead of inline checks
      'crabpal-sources/no-inline-source-auth-check': 'error',

      // Custom style rule — use z-index token scale instead of hardcoded literals
      'crabpal-styles/no-hardcoded-z-index': 'error',

      // Custom style rule — enforce approved shadow classes/tokens only
      'crabpal-styles/no-nonstandard-shadows': ['error', {
        allowedClasses: [
          'shadow-none',
          'shadow-xs',
          'shadow-minimal',
  ...
  // Temporary exceptions for unresolved shadow migrations.
  {
    ...
    rules: {
      'crabpal-styles/no-nonstandard-shadows': 'off',
    },
  },
  ...
  // Enforce backend abstraction boundary in Electron main process.
  {
    ...
    rules: {
      'no-restricted-imports': ['error', {
        paths: [
          {
            name: '@crabpal/shared/codex',
            message: 'Use provider-agnostic APIs from @crabpal/shared/agent/backend instead.',
          },
          {
            name: '@crabpal/shared/agent/claude-agent',
            message: 'Provider backends must stay behind @crabpal/shared/agent/backend.',
          },
          {
            name: '@crabpal/shared/agent/codex-agent',
            message: 'Provider backends must stay behind @crabpal/shared/agent/backend.',
          },
          {
            name: '@crabpal/shared/agent/copilot-agent',
            message: 'Provider backends must stay behind @crabpal/shared/agent/backend.',
          },
          {
            name: '@crabpal/shared/agent/pi-agent',
            message: 'Provider backends must stay behind @crabpal/shared/agent/backend.',
          },
          {
            name: '@github/copilot-sdk',
            message: 'Use provider-agnostic model discovery/validation APIs from @crabpal/shared/agent/backend.',
          },
        ],
      }],
    },
      ...
      'no-restricted-syntax': ['error',
        {
          ...
          message: 'Do not call provider APIs directly in Electron model fetchers. Delegate to fetchBackendModels() from @crabpal/shared/agent/backend.',
        },
        {
