...
<hr><center>cloudflare</center>
  ...
  it('maps 429 quota exhaustion to usage-limit messaging instead of request-throttle messaging', () => {
    const parsed = parseError(new Error('429 usage limit reached for this account; 100% usage consumed'))

    expect(parsed.code).toBe('rate_limited')
    expect(parsed.title).toBe('Usage Limit Reached')
    expect(parsed.message.toLowerCase()).toContain('usage quota')
