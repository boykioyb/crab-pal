 ...
 * Deep Link Handler
 *
 * Parses crabpal:// URLs and routes to appropriate actions.
 *
 * URL Formats (workspace is optional - uses active window if omitted):
 *
 * Compound format (hierarchical navigation):
 *   crabpal://allSessions[/session/{sessionId}]            - Session list (all sessions)
 *   crabpal://flagged[/session/{sessionId}]             - Session list (flagged filter)
 *   crabpal://state/{stateId}[/session/{sessionId}]     - Session list (state filter)
 *   crabpal://sources[/source/{sourceSlug}]          - Sources list
 *   crabpal://settings[/{subpage}]                   - Settings (general, shortcuts, preferences)
 *
 * Action format:
 *   crabpal://action/{actionName}[/{id}][?params]
 *   crabpal://workspace/{workspaceId}/action/{actionName}[?params]
 *
 * Actions:
 ...
 *   crabpal://allSessions/session/abc123                (specific session)
 *   crabpal://settings/shortcuts                     (shortcuts page)
 *   crabpal://sources/source/github                  (github source info)
 *   crabpal://action/new-chat                        (uses active window)
 *   crabpal://action/resume-sdk-session/{sdkId}      (resume Claude Code session)
 *   crabpal://workspace/ws123/allSessions/session/abc123   (targets specific workspace)
 */
...
export function parseDeepLink(url: string): DeepLinkTarget | null {
  try {
    ...
    // e.g., crabpal://workspace/ws123 → hostname='workspace', pathname='/ws123'
    // e.g., crabpal://allSessions/chat/abc → hostname='allSessions', pathname='/chat/abc'
    const host = parsed.hostname
    const pathParts = parsed.pathname.split('/').filter(Boolean)
    const windowMode = parseWindowMode(parsed)
    const rightSidebar = parseRightSidebar(parsed)

    // crabpal://auth-callback?... (OAuth callbacks - return null to let existing handler process)
    ...
    // crabpal://allSessions/..., crabpal://settings/..., etc. (compound routes)
    ...
    // crabpal://workspace/{workspaceId}/... (with workspace targeting)
    if (host === 'workspace') {
    ...
    // crabpal://action/... (no workspace - uses active window)
    if (host === 'action') {
