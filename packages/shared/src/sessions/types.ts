...
import type { StoredAttachment, MessageRole, ToolStatus, AuthRequestType, AuthStatus, CredentialInputMode, StoredMessage } from '@crabpal/core/types';
...
export const SESSION_PERSISTENT_FIELDS = [
  // Identity
  'id', 'workspaceRootPath', 'projectId', 'sdkSessionId', 'sdkCwd',
  // Timestamps
 ...
 * Re-exported from @crabpal/core for convenience
 */
export type { StoredMessage } from '@crabpal/core/types';
...
export interface SessionConfig {
   ...
   *  New sessions always carry this; legacy sessions created before the
   *  Workspace → Project → Session hierarchy may be missing it on disk. */
  projectId?: string;
...
export interface SessionHeader {
  ...
  /** SDK session ID (captured after first message) */
  sdkSessionId?: string;
  /** Workspace root path (stored as portable path, e.g., ~/.crabpal/...) */
  workspaceRootPath: string;
  /** Project (inside the workspace) this session belongs to.
   *  Always present for sessions created after the Workspace → Project → Session
   *  hierarchy was introduced; may be missing on legacy headers. */
  projectId?: string;
  /** Optional user-defined name */
  name?: string;
  createdAt: number;
...
export interface SessionMetadata {
  id: string;
  workspaceRootPath: string;
  /** Project (inside the workspace) this session belongs to. Missing on legacy sessions. */
  projectId?: string;
  name?: string;
  createdAt: number;
  lastUsedAt: number;
