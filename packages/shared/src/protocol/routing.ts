...
export const LOCAL_ONLY_CHANNELS = new Set<string>([
  ...
  // projects — workspace-local grouping of sessions. Stored under the
  // workspace rootPath, so treat identically to workspaces: for routing.
  ...
  // logs — local log file viewer
  RPC_CHANNELS.logs.READ,
  RPC_CHANNELS.logs.GET_FILE_PATH,
  RPC_CHANNELS.logs.TAIL_START,
  RPC_CHANNELS.logs.TAIL_STOP,
  RPC_CHANNELS.logs.NEW_LINES,
  ...
  // account — local account info from active agent (requires local agent access)
  RPC_CHANNELS.account.GET_INFO,
  RPC_CHANNELS.account.GET_RATE_LIMIT,

  // legacyImport — local-only migration from ~/.craft-agent/ to ~/.crabpal/
  RPC_CHANNELS.legacyImport.DETECT,
  RPC_CHANNELS.legacyImport.PREVIEW,
...
export const REMOTE_ELIGIBLE_CHANNELS = new Set<string>([
  ...
  // transfer — chunked file transfer (workspace content)
