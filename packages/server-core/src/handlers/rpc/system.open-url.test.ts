...
import type { RpcServer, HandlerFn, RequestContext } from '@crabpal/server-core/transport'
...
describe('registerSystemCoreHandlers OPEN_URL', () => {
  it('routes crabpal action links internally via deeplink:navigate', async () => {
    const { openUrl, ctx, invokeClientCalls, pushCalls } = createTestHarness()

    await openUrl(ctx, 'crabpal://action/new-session?input=sg&send=true')
  ...
  it('routes workspace deep links to workspace target when URL workspace differs', async () => {
    ...
    await openUrl(ctx, 'crabpal://workspace/ws-2/action/new-session?input=hello')
  ...
  it('falls back to client openExternal for crabpal window-mode links', async () => {
    const { openUrl, ctx, invokeClientCalls, pushCalls } = createTestHarness()

    await openUrl(ctx, 'crabpal://action/new-session?window=focused')
    ...
    expect(invokeClientCalls[0]).toEqual({
      ...
      args: ['crabpal://action/new-session?window=focused'],
    })
  })
  ...
  it('rejects unsupported protocols', async () => {
    ...
    await expect(openUrl(ctx, 'file:///tmp/test.txt')).rejects.toThrow(
      'Failed to open URL: Only http, https, mailto, craftdocs, and crabpal URLs are allowed'
    )
  })
})
