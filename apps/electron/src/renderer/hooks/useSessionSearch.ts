...
import { parseLabelEntry } from "@crabpal/shared/labels"
import { fuzzyScore } from "@crabpal/shared/search"
...
export interface UseSessionSearchOptions {
  ...
  groupingMode?: 'date' | 'status' | 'byProject'
...
function getCollapseGroupKey(
  item: SessionMeta,
  groupingMode?: 'date' | 'status' | 'byProject' | 'byProject',
): string {
  if (groupingMode === 'status') return `status-${getSessionStatus(item)}`
  if (groupingMode === 'byProject') return `project-${item.projectId ?? '__unassigned__'}`
