...
$process = Get-Process -Name "CrabPal" -ErrorAction SilentlyContinue
if ($process) {
    Write-Info "Closing CrabPal..."
...
Write-Info "Adding 'crabpal' command to PATH..."

$binDir = "$env:LOCALAPPDATA\CrabPal\bin"
$cmdFile = "$binDir\crabpal.cmd"
$exePath = "$env:LOCALAPPDATA\Programs\CrabPal\CrabPal.exe"
...
if ($userPath -notlike "*$binDir*") {
    ...
    Write-Success "Added to PATH (restart terminal to use 'crabpal' command)"
} else {
    Write-Success "Command 'crabpal' is ready"
}
...
Write-Host ""
Write-Host "  CrabPal has been installed."
Write-Host ""
Write-Host "  Launch from:"
Write-Host "    - Start Menu or desktop shortcut"
Write-Host "    - Command line: crabpal (restart terminal first)"
Write-Host ""
}
