...
export function EntityRow({
  ...
  const innerContent = (
    <div className="group relative mr-2 select-none px-2">
      {/* Main content button */}
      <button
        {...(buttonProps as React.ButtonHTMLAttributes<HTMLButtonElement>)}
        className={cn(
          "entity-row-btn flex w-full items-start gap-2 rounded-lg px-3 py-2.5 text-left text-sm outline-none",
          ...
          {titleTrailing ? (
            <div className="flex items-center gap-[10px] w-full min-w-0">
              ...
              <div className="shrink-0 ml-auto relative -mr-1">
                ...
                <div className="cursor-pointer rounded-[8px] p-1 hover:bg-foreground/10 data-[state=open]:bg-foreground/10">
      ...
      {menuContent && !hideMoreButton && !titleTrailing && (
        ...
        >
          <div className="overflow-hidden rounded-lg border border-transparent hover:border-border/50">
            <DropdownMenu modal={true} onOpenChange={setMenuOpen}>
              <DropdownMenuTrigger asChild>
