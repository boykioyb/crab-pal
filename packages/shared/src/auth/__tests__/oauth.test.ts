  ...
  describe('RFC 9728 protected resource discovery', () => {
    it('discovers metadata via WWW-Authenticate resource_metadata hint', async () => {
      const protectedResourceMetadata = {
        resource: 'https://mcp.crabpal.app/my',
        authorization_servers: ['https://mcp.crabpal.app/my/auth'],
      };

      const authServerMetadata = {
        authorization_endpoint: 'https://mcp.crabpal.app/my/auth/authorize',
        token_endpoint: 'https://mcp.crabpal.app/my/auth/token',
        registration_endpoint: 'https://mcp.crabpal.app/my/auth/register',
      };

      mockFetch.mockImplementation((url: string, options?: RequestInit) => {
        // HEAD request to MCP endpoint returns 401 with resource_metadata hint
        if (url === 'https://mcp.crabpal.app/my/mcp' && options?.method === 'HEAD') {
          return Promise.resolve(new Response(null, {
            status: 401,
            headers: {
              'WWW-Authenticate': 'Bearer error="invalid_token", resource_metadata="https://mcp.crabpal.app/.well-known/oauth-protected-resource/my"',
            },
          }));
        }
        // Protected resource metadata
        if (url === 'https://mcp.crabpal.app/.well-known/oauth-protected-resource/my') {
          return Promise.resolve(new Response(JSON.stringify(protectedResourceMetadata), { status: 200 }));
        }
        // Authorization server metadata
        if (url === 'https://mcp.crabpal.app/my/auth/.well-known/oauth-authorization-server') {
          return Promise.resolve(new Response(JSON.stringify(authServerMetadata), { status: 200 }));
        }
        return Promise.resolve(new Response('Not Found', { status: 404 }));
      });

      const result = await discoverOAuthMetadata('https://mcp.crabpal.app/my/mcp');
...
describe('prepareMcpOAuth', () => {
  ...
  it('falls back to the default client ID when registration is forbidden', async () => {
    ...
    expect(result.clientId).toBe('crabpal');
    expect(result.clientSecret).toBeUndefined();
    expect(result.authUrl).toContain('client_id=crabpal');
