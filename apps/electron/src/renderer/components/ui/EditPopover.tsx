...
import { usePlatform } from '@crabpal/ui'
...
import { activeProjectIdAtom, projectsAtom } from '@/atoms/projects'
import { activeSessionIdAtom, sessionAtomFamily } from '@/atoms/sessions'
...
const EDIT_CONFIGS: Record<EditContextKey, (location: string) => EditConfig> = {
  ...
  'default-permissions': (location) => ({
    context: {
      ...
      context:
        'The user is editing app-level default permissions (~/.crabpal/permissions/default.json). ' +
  ...
  'preferences-notes': (location) => ({
    context: {
      ...
      context:
        'The user is editing the notes field in their preferences (~/.crabpal/preferences.json). ' +
  ...
  'add-source': (location) => ({
    context: {
      ...
      context:
        ...
        'Follow the patterns in ~/.crabpal/docs/sources.md. ' +
  ...
  'add-source-local': (location) => ({
    context: {
      ...
      context:
        ...
        'First, query the built-in docs MCP server: mcp__crabpal-docs__SearchCrabPal({ query: "filesystem" }). ' +
  ...
  'add-skill': (location) => ({
    context: {
      ...
      context:
        ...
        'Follow the patterns in ~/.crabpal/docs/skills.md. ' +
  ...
  'edit-labels': (location) => ({
    context: {
      ...
      context:
        ...
        'Read ~/.crabpal/docs/labels.md for full format reference. ' +
        'Confirm clearly when done.',
    },
  ...
  'edit-tool-icons': (location) => ({
    context: {
      ...
      context:
        ...
        'The file is tool-icons.json in ~/.crabpal/tool-icons/. Icon image files live in the same directory. ' +
        ...
        'Read ~/.crabpal/docs/tool-icons.md for full format reference. ' +
  ...
  'automation-config': (location) => ({
    context: {
      ...
      context:
        ...
        'Read ~/.crabpal/docs/automations.md for full format reference. ' +
...
export function EditPopover({
  ...
  // Resolve projectId for mini-edit session creation: prefer the parent
  // (currently-viewed) session's project, fall back to the active project atom,
  // then fall back to the first project in the workspace.
  const activeSessionId = useAtomValue(activeSessionIdAtom)
  const parentSession = useAtomValue(sessionAtomFamily(activeSessionId ?? ''))
  const activeProjectId = useAtomValue(activeProjectIdAtom)
  const projects = useAtomValue(projectsAtom)
  const resolvedProjectId =
    parentSession?.projectId ?? activeProjectId ?? projects[0]?.id ?? null
    ...
    if (!sessionId && workspace?.id) {
      if (!resolvedProjectId) {
        toast.error('No project available', {
          description: 'Create a project first to enable inline edits.',
        })
        return
      }
  ...
  }, [context, inlineSessionId, workspace?.id, model, systemPromptPreset, permissionMode, workingDirectory, onCreateSession, onSendMessage, resolvedProjectId])
  ...
  const handleLegacySendMessage = useCallback((message: string) => {
    ...
    const url = `crabpal://action/new-session?input=${encodedInput}&send=true&mode=${permissionMode}&badges=${encodedBadges}${workdirParam}${modelParam}${systemPromptParam}`
