    ...
    >
      ...
      <Select value={value} onValueChange={onValueChange} disabled={disabled}>
        <SelectTrigger id={id} className={cn(
          'w-full rounded-md border border-border/40 bg-muted/40',
          'transition-all duration-150 focus-visible:ring-1 focus-visible:ring-ring/50',
  ...
  return (
    <div
      ...
      className={cn(
        ...
        inCard ? 'px-4 py-3.5 hover:bg-foreground/[0.02]' : 'py-3',
        className
      )}
    >
      ...
      <div data-layout="settings-control" className="ml-4 shrink-0">
        <Select value={value} onValueChange={onValueChange} disabled={disabled}>
          <SelectTrigger id={id} className={cn(
            'w-[180px] rounded-md border border-border/40 bg-muted/40',
            'transition-all duration-150 focus-visible:ring-1 focus-visible:ring-ring/50',
            disabled && 'bg-muted/20 cursor-not-allowed'
          )}>
            <SelectValue placeholder={placeholder} />
