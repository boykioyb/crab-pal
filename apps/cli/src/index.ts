#!/usr/bin/env bun
/**
 * crabpal — Terminal client for CrabPal server.
 *
 * Connects over WebSocket (ws:// or wss://) to a running CrabPal server
...
export function parseArgs(argv: string[]): CliArgs {
  ...
  // Env var fallbacks
  if (!url) url = process.env.CRAB_PAL_SERVER_URL ?? ''
  if (!token) token = process.env.CRAB_PAL_SERVER_TOKEN ?? ''
  if (!tlsCa) tlsCa = process.env.CRAB_PAL_TLS_CA
  ...
  return [
    ...
    },
    {
      ...
      fn: async (client, ctx) => {
        ...
        const tmpDir = await mkdtemp(`${tmpdir()}/crabpal-validate-`)
    ...
    {
      name: 'mcp:crabpal-public (auth:none)',
      fn: async (client, ctx) => {
        if (!ctx.createdSessionId) return 'skipped (no session)'
        // Enable the pre-committed crabpal-public MCP source on the session
        const enableSlugs = [ctx.createdSourceSlug, 'crabpal-public'].filter(Boolean) as string[]
        await client.invoke('sessions:command', ctx.createdSessionId, {
          type: 'setSources',
          sourceSlugs: enableSlugs,
        })
        return await waitForSendEvents(client, ctx.createdSessionId,
          `[source:crabpal-public] List the documents under the "CrabPal E2E Test" folder inside the "CrabPal" folder. Just list their names.`,
    ...
    },
    {
      ...
      fn: async (client, ctx) => {
        ...
        const enableSlugs = [ctx.createdSourceSlug, 'crabpal-public', 'stitch-mcp'].filter(Boolean) as string[]
        await client.invoke('sessions:command', ctx.createdSessionId, {
          type: 'setSources',
          sourceSlugs: enableSlugs,
        })
...
description: "Validation skill created by crabpal"
requiredSources:
  - "${sourceSlug}"
---
...
function printHelp(): void {
  process.stdout.write(`crabpal — Terminal client for CrabPal server

Usage: crabpal [options] <command> [args...]

Connection:
  --url <ws[s]://...>    Server URL (default: $CRAB_PAL_SERVER_URL)
  --token <secret>       Auth token (default: $CRAB_PAL_SERVER_TOKEN)
...
Examples:
  crabpal run "What files are in the current directory?"
  crabpal run --source crabpal-kb "Summarize today's daily note"
  crabpal run --workspace-dir .github/agents --source crabpal-public "Read the doc"
  crabpal run --provider openai --model gpt-4o "Summarize this repo"
  OPENAI_API_KEY=sk-... crabpal run --provider openai "Hello"
  GOOGLE_API_KEY=... crabpal run --provider google --model gemini-2.0-flash "Hello"
  echo "Analyze this code" | crabpal run
  crabpal ping
  crabpal sessions
  crabpal send abc-123 "What files are in the current directory?"
  echo "Summarize this" | crabpal send abc-123
...
export async function main(argv: string[] = process.argv): Promise<void> {
  ...
  if (!args.url) {
    err('No server URL. Use --url <ws://...> or set $CRAB_PAL_SERVER_URL')
    process.exit(1)
  }
