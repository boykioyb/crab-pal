...
function browserDocPath(): string {
  return resolve(join(homedir(), '.crabpal', 'docs', 'browser-tools.md'));
}
...
describe('PrerequisiteManager', () => {
  ...
  describe('rule matching', () => {
    ...
    it('exempts crabpal-docs MCP tools', () => {
      mockExistsPaths.add(guidePath('crabpal-docs'));
      const result = manager.checkPrerequisites('mcp__crabpal-docs__search');
      expect(result.allowed).toBe(true);
    });
