...
/**
 * Supported agent folder names, in priority order within a single tier.
 * `.agents` is primary; `.claude` and `.cursor` are also honored so users
 * can share skills with Claude Code and Cursor installations.
 */
export const AGENT_FOLDER_NAMES = ['.agents', '.claude', '.cursor'] as const;

/** Global agent skills directories: ~/{folder}/skills/ */
export const GLOBAL_AGENT_SKILLS_DIRS: string[] = AGENT_FOLDER_NAMES.map((folder) =>
  join(homedir(), folder, 'skills'),
);

/** Project-level agent skills relative directory names */
export const PROJECT_AGENT_SKILLS_DIRS: string[] = AGENT_FOLDER_NAMES.map(
  (folder) => `${folder}/skills`,
);

/** Global agent skills directory (primary): ~/.agents/skills/ — kept for backwards compatibility. */
export const GLOBAL_AGENT_SKILLS_DIR = GLOBAL_AGENT_SKILLS_DIRS[0]!;

/** Project-level agent skills relative directory name (primary) — kept for backwards compatibility. */
export const PROJECT_AGENT_SKILLS_DIR = PROJECT_AGENT_SKILLS_DIRS[0]!;
...
export function loadAllSkills(workspaceRoot: string, projectRoot?: string): LoadedSkill[] {
  ...
  // Within a single tier, iterate folders in priority order and only keep
  // the first occurrence of each slug (so `.agents` wins over `.claude`/`.cursor`).
  const collectTier = (dirs: string[], source: SkillSource): LoadedSkill[] => {
  ...
  // 1. Global skills (lowest priority): ~/{.agents,.claude,.cursor}/skills/
  for (const skill of collectTier(GLOBAL_AGENT_SKILLS_DIRS, 'global')) {
  ...
  // 3. Project skills (highest priority): {projectRoot}/{.agents,.claude,.cursor}/skills/
  if (projectRoot) {
    const projectDirs = PROJECT_AGENT_SKILLS_DIRS.map((rel) => join(projectRoot, rel));
    for (const skill of collectTier(projectDirs, 'project')) {
      skillsBySlug.set(skill.slug, skill);
    }
  }
...
export function loadSkillBySlug(workspaceRoot: string, slug: string, projectRoot?: string): LoadedSkill | null {
  // Highest priority: project-level (.agents > .claude > .cursor)
  if (projectRoot) {
    for (const rel of PROJECT_AGENT_SKILLS_DIRS) {
      const skill = loadSkillFromDir(join(projectRoot, rel), slug, 'project');
  ...
  // Lowest priority: global (.agents > .claude > .cursor)
  for (const dir of GLOBAL_AGENT_SKILLS_DIRS) {
    const skill = loadSkillFromDir(dir, slug, 'global');
    if (skill) return skill;
  }
  return null;
}
