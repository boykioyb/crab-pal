...
const CRAB_PAL_AGENT_CONFIG_PATTERNS = [
  ...
  /\.crabpal\/config\.json$/,
  // Preferences
  /\.crabpal\/preferences\.json$/,
  // Source configs
  /\.crabpal\/workspaces\/[^/]+\/sources\/[^/]+\/config\.json$/,
  // Permissions
  /\.crabpal\/workspaces\/[^/]+\/permissions\.json$/,
  /\.crabpal\/permissions\/[^/]+\.json$/,
  // Theme
  /\.crabpal\/workspaces\/[^/]+\/theme\.json$/,
  // Statuses
  /\.crabpal\/workspaces\/[^/]+\/statuses\/config\.json$/,
  // Labels
  /\.crabpal\/workspaces\/[^/]+\/labels\.json$/,
  // Tool icons
  /\.crabpal\/tool-icons\/tool-icons\.json$/,
];
...
export class ConfigValidator {
  ...
  /**
   * Check if a file path is a CrabPal config file.
   *
   * @param filePath - Path to check
   * @returns true if this is a CrabPal config
   */
  isCrabPalConfig(filePath: string): boolean {
    const normalizedPath = process.platform === 'win32'
      ? filePath.replace(/\\/g, '/').toLowerCase()
      : filePath.replace(/\\/g, '/');
