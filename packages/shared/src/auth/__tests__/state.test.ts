...
import {
  _getClaudeOAuthCredentialsForConnection,
  getSetupNeeds,
  performTokenRefresh,
  _resetRefreshMutex,
  type AuthState,
...
function createMockCredentialManager(initialCreds?: {
  ...
  return {
    ...
    getLlmOAuth: async (connectionSlug: string) => llmCreds.get(connectionSlug) ?? null,
    setLlmOAuth: async (connectionSlug: string, creds: {
      accessToken: string;
      refreshToken?: string;
      expiresAt?: number;
      idToken?: string;
    }) => {
      llmCreds.set(connectionSlug, creds);
    },
    deleteLlmCredentials: async (connectionSlug: string) => {
      llmCreds.delete(connectionSlug);
    },
...
describe('_getClaudeOAuthCredentialsForConnection', () => {
  it('prefers connection-scoped OAuth credentials over legacy global credentials', async () => {
    const manager = createMockCredentialManager({
    ...
    await manager.setLlmOAuth('claude-max-2', {
      accessToken: 'connection-access-token',
      refreshToken: 'connection-refresh-token',
      expiresAt: Date.now() + 60_000,
    });

    const creds = await _getClaudeOAuthCredentialsForConnection(manager as any, 'claude-max-2');

    expect(creds).toBeDefined();
    expect(creds?.accessToken).toBe('connection-access-token');
    expect(creds?.refreshToken).toBe('connection-refresh-token');
    expect(creds?.storage).toBe('llm');
    expect(creds?.source).toBe('native');
  });

  it('falls back to legacy global credentials when connection-scoped credentials are missing', async () => {
    const manager = createMockCredentialManager({
      accessToken: 'legacy-access-token',
      refreshToken: 'legacy-refresh-token',
      source: 'cli',
    ...
    expect(creds).toBeDefined();
    expect(creds?.accessToken).toBe('legacy-access-token');
    expect(creds?.refreshToken).toBe('legacy-refresh-token');
    expect(creds?.storage).toBe('legacy');
