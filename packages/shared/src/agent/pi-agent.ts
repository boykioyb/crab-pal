...
// Error typing
import { buildUsageLimitExceededError, isLikelyUsageLimitExhausted, parseError, type AgentError } from './errors.ts';
...
export class PiAgent extends BaseAgent {
  protected backendName = 'CrabPal Backend';
  ...
  // Ring buffer of recent subprocess stderr. Always on (independent of debug flags)
  // so that connection-test and other failures can surface what the subprocess
  // actually said, instead of a bare "timed out" with no context (upstream v0.8.10).
  ...
  private async spawnSubprocess(): Promise<void> {
    ...
    // connection-test timeout path) can surface it on failure. Keep the
    // debug log for interactive dev work.
   ...
   *
   * OAuth tokens from CrabPal (Claude Max, ChatGPT Plus, Copilot) are passed as
   * api_key type because they function as bearer tokens that the Pi SDK's provider
   * modules use directly. The OAuth exchange happens on the CrabPal side; by the time
  ...
  private handleSubprocessEvent(event: Record<string, unknown>): void {
    ...
    // Feed them through PiEventAdapter to convert to CrabPalEvents.
    ...
    // Adapt event to CrabPalAgentEvents
   ...
   * Uses the canonical registry from @crabpal/session-tools-core.
   */
  ...
  private parsePiError(error: Error): AgentError {
    ...
    if (errorMessage.includes('rate') || errorMessage.includes('429')) {
      if (isLikelyUsageLimitExhausted(error.message)) {
        return buildUsageLimitExceededError(error.message);
      }
      return {
        code: 'rate_limited',
        title: 'Rate Limited',
