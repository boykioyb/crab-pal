...
import { RPC_CHANNELS, type FileAttachment, type SendMessageOptions, type SessionEvent } from '@crabpal/shared/protocol'
import type { StoredAttachment } from '@crabpal/core/types'
import { getWorkspaceByNameOrId } from '@crabpal/shared/config'
import { perf } from '@crabpal/shared/utils'
import { isValidThinkingLevel } from '@crabpal/shared/agent/thinking-levels'
...
async function scanSessionDirectory(dirPath: string): Promise<import('@crabpal/shared/protocol').SessionFile[]> {
  const { readdir, stat } = await import('fs/promises')
  const entries = await readdir(dirPath, { withFileTypes: true })
  const files: import('@crabpal/shared/protocol').SessionFile[] = []
...
export function registerSessionsHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(RPC_CHANNELS.sessions.GET, async (ctx, filter?: { projectId?: string }) => {
    try {
    ...
    let sessions = sessionManager.getSessions(workspaceId ?? undefined)
    // Optional project-scoped filter (Phase 2 hierarchy: Workspace → Project → Session).
    if (filter?.projectId) {
  ...
  server.handle(RPC_CHANNELS.sessions.CREATE, async (_ctx, workspaceId: string, options?: import('@crabpal/shared/protocol').CreateSessionOptions) => {
  ...
  // Returns true if the response was delivered, false if agent/session is gone
  server.handle(RPC_CHANNELS.sessions.RESPOND_TO_CREDENTIAL, async (_ctx, sessionId: string, requestId: string, response: import('@crabpal/shared/protocol').CredentialResponse) => {
  ...
  server.handle(RPC_CHANNELS.sessions.COMMAND, async (
    _ctx,
    sessionId: string,
    command: import('@crabpal/shared/protocol').SessionCommand
  ) => {
    switch (command.type) {
      case 'flag':
  ...
  server.handle(RPC_CHANNELS.sessions.SEARCH_CONTENT, async (_ctx, workspaceId: string, query: string, searchId?: string) => {
    ...
    const { searchSessions } = await import('@crabpal/server-core/services')
    const { getWorkspaceSessionsPath } = await import('@crabpal/shared/workspaces')
  ...
  server.handle(RPC_CHANNELS.sessions.IMPORT, async (_ctx, targetWorkspaceId: string, bundle: unknown, mode: string) => {
    ...
    return sessionManager.importSession(targetWorkspaceId, bundle as import('@crabpal/shared/sessions').SessionBundle, mode)
  })
  ...
  server.handle(RPC_CHANNELS.sessions.IMPORT_REMOTE_TRANSFER, async (_ctx, targetWorkspaceId: string, payload: import('@crabpal/shared/protocol').RemoteSessionTransferPayload) => {
    await sessionManager.waitForInit()
    if (!targetWorkspaceId || typeof targetWorkspaceId !== 'string') throw new Error('targetWorkspaceId is required')
