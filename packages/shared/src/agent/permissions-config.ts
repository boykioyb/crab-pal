 ...
 *
 * File locations:
 * - Workspace: ~/.crabpal/workspaces/{slug}/permissions.json
 * - Per-source: ~/.crabpal/workspaces/{slug}/sources/{sourceSlug}/permissions.json
 *
 ...
 * Default permissions are stored at ~/.crabpal/permissions/
 * Reads env vars dynamically so tests and multi-instance dev can override the root.
 */
...
/**
 * Load default permissions from ~/.crabpal/permissions/default.json
...
class PermissionsConfigCache {
  ...
  // App-level default permissions (loaded from ~/.crabpal/permissions/default.json)
   ...
   * These come from ~/.crabpal/permissions/default.json
   */
  ...
  private applyDefaultConfig(merged: MergedPermissionsConfig, config: PermissionsCustomConfig): void {
    ...
    for (const patternEntry of config.allowedBashPatterns) {
      if (!shouldCompileBashPattern(patternEntry.pattern)) {
        debug(`[Permissions] Skipping crabpal bash pattern (feature disabled): ${patternEntry.pattern}`);
        continue;
      }
