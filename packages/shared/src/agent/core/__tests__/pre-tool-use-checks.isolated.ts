...
mock.module('../../../skills/storage.ts', () => ({
  ...
  PROJECT_AGENT_SKILLS_DIRS: ['.agents/skills', '.claude/skills', '.cursor/skills'],
}));
...
describe('runPreToolUseChecks', () => {
  ...
  describe('step 2: source blocking', () => {
    ...
    it('skips source check for built-in MCP servers (crabpal-docs)', () => {
      const result = runPreToolUseChecks(createInput({
        toolName: 'mcp__crabpal-docs__search',
        input: {},
        activeSourceSlugs: [],
      }));
  ...
  describe('step 5: input transforms', () => {
    ...
    it('blocks direct label folder reads and suggests crabpal label help when feature is enabled', () => {
      mockCrabpalCliFlag = true;
      ...
      expect(result.type).toBe('block');
      if (result.type === 'block') {
        expect(result.reason).toContain('crabpal label');
        expect(result.reason).toContain('crabpal label --help');
        expect(result.reason).toContain('labels/');
      }
    });

    it('blocks direct label config writes and suggests crabpal label help when feature is enabled', () => {
      mockCrabpalCliFlag = true;
    ...
    it('blocks direct automations config edits and suggests crabpal automation commands when feature is enabled', () => {
      mockCrabpalCliFlag = true;
      ...
      expect(result.type).toBe('block');
      if (result.type === 'block') {
        expect(result.reason).toContain('crabpal automation');
        expect(result.reason).toContain('automations.json');
      }
    });

    it('blocks direct source config edits and suggests crabpal source commands when feature is enabled', () => {
      mockCrabpalCliFlag = true;
      ...
      expect(result.type).toBe('block');
      if (result.type === 'block') {
        expect(result.reason).toContain('crabpal source');
        expect(result.reason).toContain('sources/linear/config.json');
      }
    });

    it('blocks direct skill file edits and suggests crabpal skill commands when feature is enabled', () => {
      mockCrabpalCliFlag = true;
      mockDetectConfigFileType.mockImplementation(() => ({
        type: 'skill',
        slug: 'commit-helper',
      ...
      expect(result.type).toBe('block');
      if (result.type === 'block') {
        expect(result.reason).toContain('crabpal skill');
        expect(result.reason).toContain('skills/commit-helper/SKILL.md');
      }
    });

    it('blocks bash commands touching labels paths and points to crabpal label --help when feature is enabled', () => {
      mockCrabpalCliFlag = true;
    ...
    it('allows bash crabpal label commands through labels guard', () => {
      const result = runPreToolUseChecks(createInput({
        toolName: 'Bash',
        input: { command: 'crabpal label list' },
        permissionMode: 'allow-all',
      }));
    ...
    it('blocks bash commands touching automations files and points to crabpal automation --help when feature is enabled', () => {
      mockCrabpalCliFlag = true;
      ...
      expect(result.type).toBe('block');
      if (result.type === 'block') {
        expect(result.reason).toContain('crabpal automation --help');
        expect(result.reason).toContain('crabpal automation');
      }
    });

    it('allows bash crabpal automation commands through config-domain bash guard', () => {
      const result = runPreToolUseChecks(createInput({
        toolName: 'Bash',
        input: { command: 'crabpal automation list' },
        permissionMode: 'allow-all',
      }));
