# CrabPal CLI Reference

Terminal client for CrabPal server. Connects over WebSocket (`ws://` or `wss://`) to a running headless server.
...
- For all other commands: a running CrabPal headless server with URL and token

## Installation

```bash
# Clone your CrabPal fork
...
# Option B: Link globally (adds crabpal to PATH)
...
|------|---------|---------|-------------|
| `--url <ws[s]://...>` | `CRAB_PAL_SERVER_URL` | — | Server WebSocket URL |
| `--token <secret>` | `CRAB_PAL_SERVER_TOKEN` | — | Authentication token |
| `--workspace <id>` | — | auto-detect | Workspace ID |
| `--timeout <ms>` | — | `10000` | Request timeout |
| `--tls-ca <path>` | `CRAB_PAL_TLS_CA` | — | Custom CA cert for self-signed TLS |
...
```bash
crabpal ping              # Verify connectivity (clientId + latency)
crabpal health            # Check credential store health
crabpal versions          # Show server runtime versions
```

### Resource Listing

```bash
crabpal workspaces        # List all workspaces
crabpal sessions          # List sessions in workspace
crabpal connections       # List LLM connections
crabpal sources           # List configured sources
```

### Session Operations

```bash
crabpal session create [--name <n>] [--mode <m>]  # Create session
crabpal session messages <id>                       # Print message history
crabpal session delete <id>                         # Delete session
crabpal cancel <id>                                 # Cancel processing
```
...
crabpal send <session-id> <message>

# Pipe text from stdin
echo "Summarize this file" | crabpal send <session-id>

# Read from stdin explicitly
cat document.txt | crabpal send <session-id> --stdin
```
...
crabpal invoke <channel> [json-args...]

# Subscribe to push events (Ctrl+C to stop)
crabpal listen <channel>
```

Examples:
```bash
crabpal invoke system:homeDir
crabpal invoke sessions:get '"workspace-123"'
crabpal listen session:event
```
...
```bash
crabpal run <prompt>
crabpal run --workspace-dir ./project --source github "List open PRs"
```
...
crabpal run --provider anthropic --base-url https://openrouter.ai/api/v1 --api-key $OR_KEY "Hello"
```

Prompt can also be piped via stdin:
```bash
echo "Summarize this file" | crabpal run
cat error.log | crabpal run "What's causing these errors?"
```
...
crabpal --validate-server --url ws://127.0.0.1:9100 --token <token>
...
```bash
# Get workspace IDs
WORKSPACES=$(crabpal --json workspaces | jq -r '.[].id')

# Count sessions per workspace
for ws in $WORKSPACES; do
  COUNT=$(crabpal --json --workspace "$ws" sessions | jq length)
  echo "$ws: $COUNT sessions"
done

# Create a session and capture its ID
SESSION_ID=$(crabpal --json session create --name "CI Run" | jq -r '.id')

# Send a message and wait for completion
crabpal send "$SESSION_ID" "Run the test suite and report results"

# Clean up
crabpal session delete "$SESSION_ID"
```
...
crabpal --url wss://server.example.com:9100 ping

# Self-signed certificate
crabpal --url wss://server.example.com:9100 --tls-ca /path/to/ca.pem ping
```

The `--tls-ca` flag sets `NODE_EXTRA_CA_CERTS` before connecting. You can also set `CRAB_PAL_TLS_CA` in your environment.
...
| `AUTH_FAILED` | Wrong token | Check `CRAB_PAL_SERVER_TOKEN` matches server |
