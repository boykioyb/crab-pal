 ...
 * @param workspaceRootPath - Absolute path to workspace folder (e.g., ~/.crabpal/workspaces/xxx)
 ...
 * (like crabpal-docs) that don't have filesystem folders.
 */
export function getSourcesBySlugs(workspaceRootPath: string, slugs: string[]): LoadedSource[] {
  ...
  for (const slug of slugs) {
    ...
    if (isBuiltinSource(slug)) {
      // Currently only crabpal-docs is a builtin source
      if (slug === 'crabpal-docs') {
 ...
 * Built-in sources (like crabpal-docs) are always available and merged
...
export async function createSource(
  ...
  // (bundled guides removed - agent should search crabpal-docs MCP for service-specific guidance)
