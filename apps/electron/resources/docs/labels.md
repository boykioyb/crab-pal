...
> **CLI-first workflow (recommended):** Use `crabpal label ...` commands instead of editing JSON directly.
> - `crabpal label --help`
> - Canonical command reference: [crabpal-cli.md](./crabpal-cli.md)

## Storage Locations

- Config: `~/.crabpal/workspaces/{id}/labels/config.json`
...
Prefer `crabpal` commands:

```bash
crabpal label create --name "Bug" --color "destructive"
crabpal label create --name "Priority" --color "accent" --value-type number
crabpal label create --name "Due Date" --color "info" --value-type date
crabpal label create --name "Project" --color "foreground/60"
crabpal label create --name "Alpha" --color "info" --parent-id project
crabpal label create --name "Beta" --color "success" --parent-id project
```
