...
import { getGitBashPath, setGitBashPath, clearGitBashPath } from '@crabpal/shared/config'
import { isUsableGitBashPath, validateGitBashPath } from '@crabpal/server-core/services'
import { validateFilePath } from '@crabpal/server-core/handlers'
...
} from '@crabpal/server-core/transport'
...
export function registerSystemCoreHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(RPC_CHANNELS.releaseNotes.GET, async () => {
    const { getCombinedReleaseNotes } = require('@crabpal/shared/release-notes') as typeof import('@crabpal/shared/release-notes')
    return getCombinedReleaseNotes()
  })

  server.handle(RPC_CHANNELS.releaseNotes.GET_LATEST_VERSION, async () => {
    const { getLatestReleaseVersion } = require('@crabpal/shared/release-notes') as typeof import('@crabpal/shared/release-notes')
  ...
  // Shell operations - open URL in external browser (or handle crabpal:// internally)
  server.handle(RPC_CHANNELS.shell.OPEN_URL, async (ctx, url: string) => {
    ...
    try {
      ...
      // Handle crabpal:// URLs internally via deep link handler (GUI only)
      ...
      if (!['http:', 'https:', 'mailto:', 'craftdocs:', 'crabpal:'].includes(parsed.protocol)) {
        throw new Error('Only http, https, mailto, craftdocs, and crabpal URLs are allowed')
      }
...
export function registerSystemGuiHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(RPC_CHANNELS.update.DISMISS, async (_ctx, version: string) => {
    const { setDismissedUpdateVersion } = await import('@crabpal/shared/config')
    setDismissedUpdateVersion(version)
  })

  server.handle(RPC_CHANNELS.update.GET_DISMISSED, async () => {
    const { getDismissedUpdateVersion } = await import('@crabpal/shared/config')
    return getDismissedUpdateVersion()
  })

  // Menu actions from renderer (for unified CrabPal menu)
  ...
  server.handle(RPC_CHANNELS.notification.GET_ENABLED, async () => {
    const { getNotificationsEnabled } = await import('@crabpal/shared/config/storage')
    return getNotificationsEnabled()
  })

  server.handle(RPC_CHANNELS.notification.SET_ENABLED, async (_ctx, enabled: boolean) => {
    const { setNotificationsEnabled } = await import('@crabpal/shared/config/storage')
