...
import type { RpcServer, RequestContext } from '@crabpal/server-core/transport'
...
describe('session file watcher isolation', () => {
  ...
  it('creates independent watchers per client and cleans up on disconnect', async () => {
    ...
    const { registerSessionsHandlers, cleanupSessionFileWatchForClient } = await import('@crabpal/server-core/handlers/rpc')
