...
import type { StoredAttachment, ContentBadge } from '@crabpal/core'
import { normalizePath } from '@crabpal/core/utils'
...
function InlineFileBadge({
  ...
  // Strip .crabpal workspace/session path prefix for cleaner tooltip display
  // e.g. "/Users/.../workspaces/{id}/sessions/{id}/plans/foo.md" → "plans/foo.md"
  const rawPath = badge.filePath || badge.label
  const tooltipPath = normalizePath(rawPath).replace(/^.*\.crabpal\/workspaces\/[^/]+\/(sessions\/[^/]+\/)?/, '')
...
export function UserMessageBubble({
  ...
  return (
    <div className={cn("flex flex-col items-end gap-3 w-full", className)}>
      ...
      <div
        className={cn(
          "max-w-[80%] bg-user-message-bubble rounded-lg break-words min-w-0 select-text [&_p]:m-0",
