...
if [ "$OS_TYPE" = "darwin" ]; then
    ...
    APP_BUNDLE_ID="com.boykioyb.crabpal"
    if pgrep -x "CrabPal" >/dev/null 2>&1; then
        info "Quitting CrabPal..."
        ...
        if pgrep -x "CrabPal" >/dev/null 2>&1; then
            ...
            pkill -9 -x "CrabPal" 2>/dev/null || true
    ...
    echo ""
    printf "%b\n" "  CrabPal has been installed to ${BOLD}$INSTALL_DIR/$APP_NAME${NC}"
    echo ""
    printf "%b\n" "  You can launch it from ${BOLD}Applications${NC} or by running:"
    printf "%b\n" "    ${BOLD}open -a 'CrabPal'${NC}"
    echo ""
    ...
    # New paths
    APP_DIR="$HOME/.crabpal/app"
    WRAPPER_PATH="$INSTALL_DIR/crabpal"
    APPIMAGE_INSTALL_PATH="$APP_DIR/CrabPal-x64.AppImage"

    # Kill the app if it's running
    if pgrep -f "CrabPal.*AppImage" >/dev/null 2>&1; then
        info "Stopping CrabPal..."
        pkill -f "CrabPal.*AppImage" 2>/dev/null || true
        sleep 2
    fi
...
#!/bin/bash
# CrabPal launcher - handles Linux-specific AppImage issues

APPIMAGE_PATH="$HOME/.crabpal/app/CrabPal-x64.AppImage"
ELECTRON_CACHE="$HOME/.config/@crabpal"
ELECTRON_CACHE_ALT="$HOME/.cache/@crabpal"
...
# AppImage creates a new /tmp/.mount_CrabPal-XXXX each launch, so any cached path is stale
for cache_dir in "$ELECTRON_CACHE" "$ELECTRON_CACHE_ALT"; do
    if [ -d "$cache_dir" ] && grep -rq '/tmp/\.mount_CrabPal' "$cache_dir" 2>/dev/null; then
        rm -rf "$cache_dir"
    fi
done
...
WRAPPER_EOF
    ...
    OLD_APPIMAGE="$INSTALL_DIR/CrabPal-x64.AppImage"
