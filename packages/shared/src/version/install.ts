...
export async function installArchive(params: { archiveData: ArrayBuffer, version: string }): Promise<void> {
  ...
  const versionDirectory = join(homedir(), '.local', 'share', 'crabpal', 'versions', version);
  const binaryPath = join(versionDirectory, 'crabpal');
  const symlinkDirectory = join(homedir(), '.local', 'bin');
  const symlinkPath = join(symlinkDirectory, 'crabpal');
