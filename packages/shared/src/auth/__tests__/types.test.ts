...
describe('buildOAuthDeeplinkUrl', () => {
  it('returns deeplink URL when sessionId and deeplinkScheme are provided', () => {
    const ctx: OAuthSessionContext = {
      ...
      deeplinkScheme: 'crabpal',
    };
    expect(buildOAuthDeeplinkUrl(ctx)).toBe('crabpal://allSessions/session/260209-swift-river');
  });
  ...
  it('uses custom deeplink scheme for multi-instance', () => {
    const ctx: OAuthSessionContext = {
      ...
      deeplinkScheme: 'crabpal1',
    };
    expect(buildOAuthDeeplinkUrl(ctx)).toBe('crabpal1://allSessions/session/test-session');
  });
  ...
  it('handles session ID with special characters (hyphens and numbers)', () => {
    ...
    expect(buildOAuthDeeplinkUrl(ctx)).toBe('crabpal://allSessions/session/260209-swift-river-42');
  });
  ...
  it('handles session ID with URL-unsafe characters', () => {
    ...
    expect(buildOAuthDeeplinkUrl(ctx)).toBe('crabpal://allSessions/session/session/with spaces&special=chars');
  });
  ...
  it('constructs correct URL format for instance 2', () => {
    const ctx: OAuthSessionContext = {
      ...
      deeplinkScheme: 'crabpal2',
    };
    expect(buildOAuthDeeplinkUrl(ctx)).toBe('crabpal2://allSessions/session/260111-bold-moon');
  });
});
