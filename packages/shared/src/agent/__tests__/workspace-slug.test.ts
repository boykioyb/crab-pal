...
describe('readPluginName', () => {
  ...
  it('reads plugin name from .claude-plugin/plugin.json', () => {
    ...
    writeFileSync(join(wsDir, '.claude-plugin', 'plugin.json'), JSON.stringify({ name: 'crabpal-workspace-default', version: '1.0.0' }))
    expect(readPluginName(wsDir)).toBe('crabpal-workspace-default')
  })
...
describe('workspace slug extraction', () => {
  ...
  it('reads plugin name from plugin.json when available', () => {
    ...
    mkdirSync(join(wsDir, '.claude-plugin'), { recursive: true })
    writeFileSync(join(wsDir, '.claude-plugin', 'plugin.json'), JSON.stringify({ name: 'crabpal-workspace-default', version: '1.0.0' }))
    expect(extractWorkspaceSlug(wsDir, fallback)).toBe('crabpal-workspace-default')
  ...
  it('handles Windows-style paths with backslashes', () => {
    expect(extractWorkspaceSlug('C:\\Users\\ghalmos\\.crabpal\\workspaces\\my-workspace', fallback)).toBe('my-workspace')
  })

  it('handles Windows paths with tilde and backslashes', () => {
    expect(extractWorkspaceSlug('~\\.crabpal\\workspaces\\my-workspace', fallback)).toBe('my-workspace')
  })
