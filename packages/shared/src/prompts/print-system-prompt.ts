#!/usr/bin/env bun
/**
 * Debug script to print the full CrabPal system prompt with annotations.
...
${colors.bgMagenta}${colors.bold}                    CRABPAL SYSTEM PROMPT BREAKDOWN                         ${colors.reset}
${colors.bgMagenta}${colors.bold}                                                                                ${colors.reset}
`);
...
printAnnotation('  2. CrabPal Environment Marker - version, platform, arch');
...
const systemPrompt = getSystemPrompt(
  ...
  '/Users/example/.crabpal/workspaces/abc123' // Example workspace path
);
...
const systemPromptWithDebug = getSystemPrompt(
  undefined,
  { enabled: true, logFilePath: '~/Library/Logs/@crabpal/electron/main.log' },
  '/Users/example/.crabpal/workspaces/abc123'
);
...
const sessionState = formatSessionState('260121-example-session', {
  plansFolderPath:
    '/Users/example/.crabpal/workspaces/abc123/sessions/260121-example-session/plans',
});
...
printAnnotation('Generated by ClaudeAgent.formatSourceState() - requires agent instance');
...
${colors.bold}SDK Configuration:${colors.reset}
  ...
  systemPrompt.append: getSystemPrompt() ${colors.dim}// CrabPal additions (static, cacheable)${colors.reset}
...
${colors.bold}Static System Prompt Components:${colors.reset}
  ...
  2. CrabPal Environment Marker      ${colors.dim}// Version, platform, arch${colors.reset}
...
${colors.bold}Key Files:${colors.reset}
  ...
  packages/shared/src/agent/crabpal.ts       ${colors.dim}// User message building${colors.reset}
