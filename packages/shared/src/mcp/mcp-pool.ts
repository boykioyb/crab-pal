...
import { CrabPalMcpClient, type McpClientConfig, type PoolClient } from './client.ts';
...
/**
 * Convert SdkMcpServerConfig (used by backend types) to CrabPalMcpClient config.
 */
...
export class McpClientPool {
  ...
  protected async registerClient(slug: string, client: PoolClient): Promise<void> {
    // listTools() triggers connect() internally for both CrabPalMcpClient and ApiSourcePoolClient
  ...
  async connect(slug: string, config: SdkMcpServerConfig): Promise<void> {
    ...
    await this.registerClient(slug, new CrabPalMcpClient(clientConfig));
