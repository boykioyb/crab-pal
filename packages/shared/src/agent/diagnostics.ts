...
function getProviderLabelFromType(providerType?: LlmProviderType, baseUrl?: string): string {
  if (providerType) {
    switch (providerType) {
      ...
      case 'pi':
      case 'pi_compat': return 'CrabPal Backend';
    }
  }
    ...
    try {
      ...
      if (fetchError instanceof Error && fetchError.name === 'AbortError') {
        return {
          ...
          failMessage: 'Cannot connect to the CrabPal MCP server (timeout). Check your network connection.',
        };
      }
      const msg = fetchError instanceof Error ? fetchError.message : String(fetchError);
      ...
      if (msg.includes('ECONNREFUSED') || msg.includes('ENOTFOUND') || msg.includes('fetch failed')) {
        return {
          ...
          failCode: 'mcp_unreachable',
          failTitle: 'MCP Server Unreachable',
          failMessage: 'Cannot connect to the CrabPal MCP server. Check your network connection.',
        };
      }
