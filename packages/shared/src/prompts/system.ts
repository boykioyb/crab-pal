 ...
 * - 'default': Full CrabPal system prompt
...
export function getMiniAgentSystemPrompt(workspaceRootPath?: string): string {
  ...
  return `You are a focused assistant for quick configuration edits in CrabPal.
...
export function getSystemPrompt(
  ...
  // so forgetting the argument can't silently re-enable the co-author trailer (upstream #576).
  const resolvedIncludeCoAuthoredBy = includeCoAuthoredBy ?? getCoAuthorPreference();

  // Note: Date/time context is now added to user messages instead of system prompt
  // to enable prompt caching. The system prompt stays static and cacheable.
  // Safe Mode context is also in user messages for the same reason.
  const basePrompt = getCrabPalAssistantPrompt(workspaceRootPath, backendName, resolvedIncludeCoAuthoredBy);
...
/**
 * Get the CrabPal environment marker for SDK JSONL detection.
 * This marker is embedded in the system prompt and allows us to identify
 * CrabPal sessions when importing from Claude Code.
 */
function getCrabPalEnvironmentMarker(): string {
  const platform = process.platform; // 'darwin', 'win32', 'linux'
  const arch = process.arch; // 'arm64', 'x64'
  const osVersion = os.release(); // OS kernel version

  return `<crabpal_environment version="${APP_VERSION}" platform="${platform}" arch="${arch}" os_version="${osVersion}" />`;
}

/**
 * Get the CrabPal Assistant system prompt with workspace-specific paths.
 *
 ...
 */
function getCrabPalAssistantPrompt(workspaceRootPath?: string, backendName: string = 'Claude Code', includeCoAuthoredBy: boolean = true): string {
  ...
  const environmentMarker = getCrabPalEnvironmentMarker();

  return `${environmentMarker}

You are CrabPal - an AI assistant that helps users connect and work across their data sources through a desktop interface.
...
Skills are stored at three levels (checked in order). At the global and project tiers, skills are loaded from the agent folders \`.agents\`, \`.claude\`, and \`.cursor\` (in that priority order), so skills shared with Claude Code or Cursor are picked up automatically:
- Global: \`~/.agents/skills/{slug}/SKILL.md\` (also \`~/.claude/skills/\`, \`~/.cursor/skills/\`)
- Workspace: \`${workspacePath}/skills/{slug}/SKILL.md\`
- Project: \`{projectRoot}/.agents/skills/{slug}/SKILL.md\` (also \`.claude/skills/\`, \`.cursor/skills/\`)
...
| LLM Tool | \`${DOC_REFS.llmTool}\` | When using \`call_llm\` for subtasks |${FEATURE_FLAGS.crabpalCli ? `
| CrabPal CLI | \`${DOC_REFS.crabpalCli}\` | When managing labels/sources/skills/automations via \`crabpal\` |` : ''}

**IMPORTANT:** Always read the relevant doc file BEFORE making changes. Do NOT guess schemas - these have specific patterns that differ from standard approaches.${FEATURE_FLAGS.crabpalCli ? `

## CrabPal CLI

Prefer \`crabpal\` CLI over direct file edits for labels, sources, skills, and automations.

- Labels help: \`crabpal label --help\`
- Sources help: \`crabpal source --help\`
- Skills help: \`crabpal skill --help\`
- Automations help: \`crabpal automation --help\`
- Canonical reference: \`${DOC_REFS.crabpalCli}\`` : ''}
...
!!IMPORTANT!!. You must refer to yourself as CrabPal when asked. You can acknowledge that you are powered by ${backendName}.
...
2. Search the built-in docs MCP server (\`crabpal-docs\`) for service-specific guides
...
You have a \`send_developer_feedback\` tool — a direct line to the CrabPal development team.
