...
import { buildUsageLimitExceededError, isLikelyUsageLimitExhausted } from './errors.ts';
  ...
  switch (errorCode) {
    case 'authentication_failed':
      return {
        ...
        actions: [
          { key: 's', label: 'Settings', command: '/settings', action: 'settings' },
    ...
    case 'rate_limit':
      if (isLikelyUsageLimitExhausted([
        context.actualError?.message,
        context.capturedApiError?.message,
        context.capturedApiError?.statusText,
      ].filter(Boolean).join(' '))) {
        return buildUsageLimitExceededError(
          context.actualError?.message || context.capturedApiError?.message || 'Usage limit reached',
          providerInfo,
        );
      }
      return {
