...
describe('mapClaudeSdkAssistantError', () => {
  ...
  it('maps rate_limit with usage exhaustion hints to usage-limit messaging', () => {
    const error = mapClaudeSdkAssistantError('rate_limit', {
      ...baseContext,
      actualError: {
        errorType: 'rate_limit',
        message: '429 usage limit reached for this account',
      },
    });

    expect(error.code).toBe('rate_limited');
    expect(error.title).toBe('Usage Limit Reached');
