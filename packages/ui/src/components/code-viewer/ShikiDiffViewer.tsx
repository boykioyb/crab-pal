...
import { registerCrabPalShikiThemes } from './registerShikiThemes'
...
export interface ShikiDiffViewerProps {
   ...
   *  Shiki theme natively. Falls back to crabpal-dark/crabpal-light (transparent bg) if not set. */
  shikiTheme?: string
...
export function ShikiDiffViewer({
  ...
  // to crabpal-dark/crabpal-light which have transparent bg for CSS variable theming
  const resolvedThemeName = shikiTheme || (theme === 'dark' ? 'crabpal-dark' : 'crabpal-light')
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      ;(header as any).__crabpalClickCleanup = () => header.removeEventListener('click', handleClick)
    }, 150)
    ...
    return () => {
      ...
      const header = diffsContainer?.shadowRoot?.querySelector('[data-diffs-header]')
      if (header) {
        ;(header as any).__crabpalClickCleanup?.()
      }
    }
