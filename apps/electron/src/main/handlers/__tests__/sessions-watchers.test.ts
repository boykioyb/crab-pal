...
import { registerSessionsHandlers, cleanupSessionFileWatchForClient } from '@crabpal/server-core/handlers/rpc'
...
describe('sessions file watchers', () => {
  ...
  beforeEach(() => {
    ...
    tempRoot = mkdtempSync(join(tmpdir(), 'crabpal-session-watchers-'))
