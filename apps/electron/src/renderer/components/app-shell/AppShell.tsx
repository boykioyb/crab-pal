...
import { Tooltip, TooltipTrigger, TooltipContent, DocumentFormattedMarkdownOverlay } from "@crabpal/ui"
import {
  DropdownMenu,
  DropdownMenuTrigger,
  DropdownMenuSub,
...
import { SessionList, type ChatGroupingMode, type SessionListHandle } from "./SessionList"
...
import { getEffectiveWorkingDirectory } from "@crabpal/core/utils"
...
import { buildLabelTree, getDescendantIds, getLabelDisplayName, flattenLabels, extractLabelId, findLabelById, sortLabelsForDisplay } from "@crabpal/shared/labels"
import type { LabelConfig, LabelTreeNode } from "@crabpal/shared/labels"
import { resolveEntityColor } from "@crabpal/shared/colors"
...
function AppShellContent({
  ...
  // Sidebar can be full-width (with labels) or collapsed to an icon rail.
  // Defaults to full-width — user toggles collapse via the chevron button.
  const [isSidebarCollapsed, setIsSidebarCollapsed] = React.useState(() => {
    return storage.get(storage.KEYS.sidebarCollapsed, false)
  })
  React.useEffect(() => {
    storage.set(storage.KEYS.sidebarCollapsed, isSidebarCollapsed)
  }, [isSidebarCollapsed])
  ...
  const [isResizing, setIsResizing] = React.useState<'session-list' | null>(null)
  ...
  const chatGroupingMode: ChatGroupingMode = isStateSubView
    ? 'date'
    : (viewFiltersMap[sessionFilterKey ?? '']?.groupingMode ?? 'byProject')
  ...
  // Imperative handle to the SessionList (byProject mode) — lets the Chats
  // section header in the sidebar drive collapse-all / expand-all / focus-search.
  const sessionListApiRef = React.useRef<SessionListHandle>(null)
  ...
  React.useEffect(() => {
    const handleGlobalPaste = (e: ClipboardEvent) => {
      ...
      if (!targetSessionId) return
      window.dispatchEvent(new CustomEvent('crabpal:paste-files', {
  ...
  // Reload skills when active session's effective working directory changes
  // (for project-level skills). Skills are loaded from: global (~/.agents/skills/),
  // workspace, and project ({effectiveWorkingDirectory}/.agents/skills/). The
  // effective directory is the session-level override when set, otherwise the
  // owning project's rootPath — see getEffectiveWorkingDirectory.
  ...
  const activeSessionWorkingDirectory = React.useMemo(() => {
    ...
    const project = meta.projectId
      ...
      : null
    if (!project) return meta.workingDirectory
    return getEffectiveWorkingDirectory(meta, project)
  }, [session.selected, sessionMetaMap, projects])
  React.useEffect(() => {
    if (!activeWorkspaceId) return
  ...
  return (
    <AppShellProvider value={appShellContextValue}>
      ...
      <div
        ref={shellRef}
        className="relative flex items-stretch bg-[radial-gradient(circle_at_top_left,rgba(13,148,136,0.08),transparent_30%),radial-gradient(circle_at_top_right,rgba(245,158,11,0.06),transparent_24%)]"
        style={{ height: '100%', paddingRight: PANEL_EDGE_INSET, paddingBottom: PANEL_EDGE_INSET, paddingLeft: PANEL_EDGE_INSET, gap: PANEL_GAP }}
      >
        <PanelStackContainer
          sidebarSlot={
            <div
            ...
            >
            <div className="flex h-full flex-col rounded-lg border border-foreground/8 bg-[linear-gradient(180deg,color-mix(in_oklch,var(--background)_92%,white_8%)_0%,color-mix(in_oklch,var(--background)_98%,black_2%)_100%)] py-2 shadow-[0_28px_70px_-48px_rgba(15,23,42,0.55)] backdrop-blur-sm select-none overflow-hidden">
              {/* Icon Rail: centered icons with tooltips */}
              <div className="flex-1 flex flex-col min-h-0 overflow-y-auto">
                ...
                <LeftSidebar
                  ...
                  links={[
                    // --- Codex-style top nav ---
                    // "New chat" — reuses the existing handleNewChat flow.
                    {
                      id: "nav:newChat",
                      title: "New chat",
                    ...
                    // "Search" — activates session search mode, navigates to All
                    // Sessions if needed, and focuses the SessionList search input.
                    {
                      id: "nav:search",
                      title: "Search",
                      icon: Search,
                      ...
                      onClick: () => {
                        ...
                        // Deferred: the SessionList input only exists once searchActive=true,
                        // so we rAF through the imperative handle.
                        requestAnimationFrame(() => sessionListApiRef.current?.focusSearchInput())
                      },
                    },
                    // --- All Sessions: expandable with status children + Flagged & Archived ---
                    {
                      id: "nav:allSessions",
                      title: "All Sessions",
                    ...
                    // --- Separator ---
                    { id: "separator:chats-sources", type: "separator" as const },
                    ...
                    // --- Separator before Chats (separator style + section
                    //     header are rendered below in the "Chats" block) ---
                    // --- Settings --- (pinned at bottom via separate slot below)
                  ]}
                />
              ...
              {/* What's New — pinned at the bottom of the sidebar. */}
              <div className="px-2 pt-1 shrink-0 border-t border-foreground/6">
                <button
                  onClick={handleWhatsNewClick}
                  className={cn(
                    "w-full flex items-center h-8 rounded-lg transition-colors duration-150 outline-none focus-visible:ring-1 focus-visible:ring-inset focus-visible:ring-ring",
                    "text-muted-foreground/60 hover:text-foreground hover:bg-foreground/5",
                    isSidebarCollapsed ? "justify-center" : "px-2.5 gap-2.5"
                  )}
                ...
                >
                  ...
                  {!isSidebarCollapsed && (
                    <span className="text-[12px] font-medium truncate">What's New</span>
                  )}
                </button>
              </div>

              {/* Settings — pinned at the bottom of the sidebar (Codex-style). */}
              <div className="px-2 pt-1 shrink-0">
                <button
                  onClick={() => handleSettingsClick('app')}
                  data-active={isSettingsNavigation(navState)}
                  className={cn(
                    "w-full flex items-center h-8 rounded-lg transition-colors duration-150 outline-none focus-visible:ring-1 focus-visible:ring-inset focus-visible:ring-ring",
                ...
                >
                  ...
                  {!isSidebarCollapsed && (
                    <span className="text-[12px] font-medium truncate">Settings</span>
                  )}
                </button>
              </div>
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button
                      ...
                      className={cn(
                        "w-full flex items-center h-8 rounded-lg transition-colors duration-150 text-muted-foreground/60 hover:text-foreground hover:bg-foreground/5 outline-none focus-visible:ring-1 focus-visible:ring-inset focus-visible:ring-ring",
                        isSidebarCollapsed ? "justify-center" : "px-2.5 gap-2.5"
                      )}
                      aria-label={isSidebarCollapsed ? 'Expand sidebar' : 'Collapse sidebar'}
                    >
                      {isSidebarCollapsed
                        ? <PanelLeftOpen className="h-[16px] w-[16px] shrink-0" />
                        : <PanelLeftClose className="h-[16px] w-[16px] shrink-0" />}
                      {!isSidebarCollapsed && (
                        <span className="text-[12px] font-medium truncate">Collapse</span>
                      )}
                    </button>
                  </TooltipTrigger>
          ...
          navigatorSlot={
            <div
              ...
              className="relative z-panel flex h-full min-w-0 flex-col rounded-lg border border-foreground/8 bg-background/92 shadow-[0_24px_64px_-42px_rgba(15,23,42,0.55)] backdrop-blur-sm"
            >
            <PanelHeader
                            ...
                            {!isStateSubView && (
                              <>
                                ...
                                <DropdownMenuSub>
                                  ...
                                  <StyledDropdownMenuSubContent minWidth="min-w-[140px]">
                                    <StyledDropdownMenuItem onClick={() => setChatGroupingMode('byProject')}>
                                      <Layers className="h-3.5 w-3.5" />
                                      <span className="flex-1">Project</span>
                                      {chatGroupingMode === 'byProject' && <Check className="h-3 w-3 text-muted-foreground" />}
                                    </StyledDropdownMenuItem>
            ...
            {isSessionsNavigation(navState) && (
              /* Sessions List (Codex IA — projects are the grouping,
                 All/Flagged/Archived chips removed; status chips were
                 redundant now that projects are the top-level grouping.
                 Filter dropdown in PanelHeader still handles label/status
                 filtering for power users. */
              <>
                {/* "Chats" section header with 3 action icons:
                    collapse-all (toggles collapse/expand of all projects),
                    filter (reuses the PanelHeader filter dropdown via a
                    TODO for Phase B unification — for now keep simple),
                    new project (opens NewProjectDialog). */}
                <div className="flex items-center gap-0.5 px-3 pb-1.5 shrink-0">
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <button
                        onClick={() => {
                          // Toggle behavior: if anything is collapsed, expand all;
                          // otherwise collapse all.
                          sessionListApiRef.current?.collapseAll()
                        }}
                        className="h-5 w-5 flex items-center justify-center rounded text-muted-foreground hover:bg-foreground/5 hover:text-foreground transition-colors"
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <button
                        onClick={() => {
                          sessionListApiRef.current?.expandAll()
                        }}
                        className="h-5 w-5 flex items-center justify-center rounded text-muted-foreground hover:bg-foreground/5 hover:text-foreground transition-colors"
        ...
        {/* Sidebar resize handle removed — icon rail is fixed 48px */}
