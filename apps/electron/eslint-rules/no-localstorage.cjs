 ...
 *
 * Warns against using localStorage in CrabPal codebase.
 * All persistent user settings should be stored in file-based configs
 * (preferences.json, workspace configs) for consistency with CrabPal
...
module.exports = {
  meta: {
    ...
    messages: {
      noLocalStorage:
        "Avoid localStorage in CrabPal. Store settings in ~/.crabpal/preferences.json using window.electronAPI.readPreferences/writePreferences. See packages/shared/src/config/preferences.ts for the preferences API.",
    },
    schema: [],
  },
