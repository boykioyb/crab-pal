...
import { Tooltip, TooltipTrigger, TooltipContent } from "@crabpal/ui"
...
import { getDocUrl } from "@crabpal/shared/docs/doc-links"
...
export function TopBar({
  ...
  return (
    <div
      className="fixed top-0 left-0 right-0 h-[48px] z-panel px-3 pt-2 titlebar-drag-region"
    >
      <div className="mx-auto flex h-[40px] w-full items-center justify-between gap-2 rounded-lg border border-foreground/8 bg-background/88 px-2 shadow-minimal backdrop-blur-xl">
      ...
      <div className="pointer-events-auto flex min-w-0 flex-1 items-center gap-0.5" style={{ paddingLeft: menuLeftPadding }}>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button
              type="button"
              aria-label="CrabPal menu"
              className="titlebar-no-drag flex h-[30px] items-center gap-2 rounded-lg border border-foreground/8 bg-foreground/[0.03] pl-2 pr-3 text-left transition-colors hover:bg-foreground/[0.05]"
            >
              <CrabPalBrandMark size="xs" variant="soft" />
              <span className="flex min-w-0 flex-col leading-none">
                <span className="text-[10px] font-semibold uppercase tracking-[0.18em] text-foreground/45">CrabPal</span>
                <span className="max-w-[112px] truncate text-[12px] font-medium text-foreground/80">
        ...
        <div className={cn("ml-1 flex min-w-0 items-center gap-1 rounded-lg bg-foreground/[0.03] px-1.5 py-1", isCompact ? "flex-1" : "w-[clamp(240px,42vw,640px)]")}>
          <Tooltip>
            <TooltipTrigger asChild>
      ...
      {!isCompact && (
      <div ref={rightSlotRef} className="flex min-w-0 shrink-0 items-center justify-end gap-1 rounded-lg bg-foreground/[0.03] px-1.5 py-1" style={{ paddingRight: 8 }}>
