...
import type { ToolDisplayMeta, AnnotationV1 } from '@crabpal/core'
import { normalizePath, pathStartsWith, stripPathPrefix } from '@crabpal/core/utils'
        ...
        >
          ...
          {onOpenDetails && isComplete && (
            <div
              ...
              className={cn(
                "p-0.5 rounded opacity-0 group-hover/row:opacity-100 transition-opacity duration-150 shrink-0",
                "hover:bg-foreground/5 focus:outline-none focus-visible:ring-1 focus-visible:ring-ring/50"
              )}
            >
...
function ActivityGroupRow({ group, expandedGroups: externalExpandedGroups, onExpandedGroupsChange, onOpenActivityDetails, animationIndex = 0, sessionFolderPath, displayMode = 'detailed' }: ActivityGroupRowProps) {
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "group/row flex items-center gap-2 py-0.5 rounded-lg cursor-pointer text-muted-foreground",
...
export function ResponseCard({
  ...
  if (isCompleted || variant === 'plan') {
    ...
    return (
      <>
        <div className="bg-background/80 backdrop-blur-sm border border-border/30 shadow-sm rounded-lg overflow-hidden relative group">
          {/* Fullscreen button - desktop only; compact mode keeps message chrome minimal */}
          {!compactMode && (
          <button
            onClick={() => setIsFullscreen(true)}
            className={cn(
              "absolute top-2 right-2 p-1 rounded-md transition-all duration-150 z-10 select-none",
              "opacity-0 group-hover:opacity-100",
              "bg-background/80 backdrop-blur-sm shadow-sm border border-border/30",
              "text-muted-foreground/50 hover:text-foreground",
              "focus:outline-none focus-visible:ring-1 focus-visible:ring-ring/50 focus-visible:opacity-100"
            )}
          ...
          {isPlan && (
            <div
              className={cn(
                "px-4 py-2 border-b border-border/30 flex items-center gap-2 bg-success/[0.05] select-none",
                SIZE_CONFIG.fontSize
              )}
            >
          ...
          {!compactMode && (
            <div className={cn(
              "pl-4 pr-2.5 py-2 border-t border-border/30 flex items-center justify-between bg-foreground/[0.025]",
              SIZE_CONFIG.fontSize
            )}>
  ...
  return (
    <>
      <div className="bg-background/80 backdrop-blur-sm border border-border/30 shadow-sm rounded-lg overflow-hidden group">
        ...
        {!compactMode && (
          <div className={cn("px-4 py-2 border-t border-border/30 flex items-center bg-foreground/[0.025]", SIZE_CONFIG.fontSize)}>
            ...
            {isExpanded && (
              ...
              >
                ...
                <div
                  ...
                  className={cn(
                    "pl-4 pr-2 py-0 space-y-0.5 border-l-2 border-border/30 ml-[13px]",
