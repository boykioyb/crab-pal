...
const ERROR_DEFINITIONS: Record<ErrorCode, Omit<AgentError, 'code' | 'originalError' | 'details'>> = {
  ...
  mcp_unreachable: {
    ...
    message: 'Cannot connect to the CrabPal MCP server. Check your network connection.',
    actions: [
      { key: 'r', label: 'Retry', action: 'retry' },
    ],
    canRetry: true,
  ...
  model_no_tool_support: {
    ...
    message: 'The selected model does not support tool/function calling, which is required for CrabPal. Please choose a model with tool support (e.g., Claude, GPT-4, Gemini).',
    actions: [
...
export function isLikelyUsageLimitExhausted(message: string): boolean {
  const lower = message.toLowerCase();
  return USAGE_LIMIT_HINTS.some((hint) => lower.includes(hint));
}

export function buildUsageLimitExceededError(
  originalError: string,
  providerInfo?: ProviderInfo,
): AgentError {
  return {
    ...
    message: 'This account has exhausted its current usage quota. Switch to another connection or wait for the quota to reset.',
    actions: [
      { key: 's', label: 'Open settings', command: '/settings', action: 'settings' },
    ],
    canRetry: false,
...
export function parseError(
  ...
  if (code === 'rate_limited' && isLikelyUsageLimitExhausted(fullErrorText)) {
    return buildUsageLimitExceededError(errorMessage, providerInfo);
  }
  ...
  if (code === 'model_no_tool_support') {
    ...
    if (modelMatch?.[1]) {
      return {
        code,
        ...definition,
        message: `Model "${modelMatch[1]}" does not support tool/function calling, which is required for CrabPal. Please choose a different model with tool support in Settings.`,
        originalError: errorMessage,
        providerInfo,
      };
    }
