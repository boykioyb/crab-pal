...
// Mutex to prevent concurrent token refresh attempts per connection.
// Different Claude OAuth connections must not block or overwrite each other.
...
async function getClaudeOAuthCredentialsForConnection(
...
): Promise<ClaudeOAuthCredentialRecord | null> {
  ...
  if (llmCreds?.accessToken) {
    return {
      ...
      // Connection-scoped Claude OAuth credentials are always our native flow.
      source: 'native',
      storage: 'llm',
    };
  }

  const legacyCreds = await manager.getClaudeOAuthCredentials();
  if (!legacyCreds?.accessToken) return null;

  return {
    accessToken: legacyCreds.accessToken,
    refreshToken: legacyCreds.refreshToken,
    expiresAt: legacyCreds.expiresAt,
    source: legacyCreds.source,
    storage: 'legacy',
  };
}
...
export async function performTokenRefresh(
  ...
  try {
    ...
    // Always refresh the connection-scoped credential actually in use.
    ...
    // Keep legacy global credential in sync only when we explicitly fell back to it.
    if (options?.syncLegacyCredential) {
    ...
    if (isIncompatibleToken) {
      ...
      // Clear the connection-scoped credential that actually failed.
...
export async function getValidClaudeOAuthToken(connectionSlug: string): Promise<TokenResult> {
  ...
  // Prefer connection-scoped OAuth. Legacy global credentials are only a fallback.
  ...
  if (isTokenExpired(creds.expiresAt)) {
    ...
    if (creds.refreshToken) {
      ...
      const refreshPromise = performTokenRefresh(manager, creds.refreshToken, creds.source, connectionSlug, {
        syncLegacyCredential: creds.storage === 'legacy',
      });
      refreshInProgress.set(connectionSlug, refreshPromise);
      ...
      try {
        ...
        // Release the mutex
        if (refreshInProgress.get(connectionSlug) === refreshPromise) {
...
export const _getClaudeOAuthCredentialsForConnection = getClaudeOAuthCredentialsForConnection;
