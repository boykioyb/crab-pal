...
import { TooltipProvider } from '@crabpal/ui'
...
import { extractWorkspaceSlugFromPath } from '@crabpal/shared/utils/workspace-slug'
import { DEFAULT_THINKING_LEVEL } from '@crabpal/shared/agent/thinking-levels'
...
import { loadProjectsAtom, subscribeProjectsChanged } from '@/atoms/projects'
...
function extractSlashCommand(message: string): string | null {
  const match = message.trim().match(/^\/([a-z]+)(?:\s|$)/i)
  return match?.[1]?.toLowerCase() ?? null
}

function buildSlashCommandBadge(commandId: string, rawText: string): ContentBadge {
  const label =
    commandId === 'compact' ? 'Compact' : commandId.charAt(0).toUpperCase() + commandId.slice(1)
    ...
    <div className="flex h-full items-center justify-center p-6">
      <div className="max-w-lg rounded-xl border border-border/50 bg-background shadow-minimal p-6 text-center">
        ...
        <p className="mt-2 text-sm text-foreground/60">
          CrabPal could connect, but loading the session list failed. This is not being treated as an empty workspace to avoid hiding a real bug or data problem.
        </p>
...
export default function App() {
  ...
  useEffect(() => {
    const initialize = async () => {
      try {
        ...
        if (needs.isFullyConfigured) {
          // If no workspace is selected (thin client without CRABPAL_WORKSPACE_ID),
  ...
  // Load projects and subscribe to projects:changed for the active workspace.
  // The atom module is kept side-effect-free — the subscription is wired here.
  const loadProjects = useSetAtom(loadProjectsAtom)
  useEffect(() => {
    if (!windowWorkspaceId) return
    const wsId = windowWorkspaceId
    void loadProjects(wsId).catch(err => {
      console.error('[App] Failed to load projects:', err)
    })
    const unsubscribe = subscribeProjectsChanged(wsId, () => {
      void loadProjects(wsId).catch(err => {
        console.error('[App] Failed to reload projects after change:', err)
      })
    })
    return () => {
      unsubscribe()
    }
  }, [windowWorkspaceId, loadProjects])
  ...
  useEffect(() => {
      ...
      for (const effect of effects) {
        switch (effect.type) {
          ...
          case 'restore_input': {
            ...
            window.dispatchEvent(new CustomEvent('crabpal:restore-input', {
    ...
    const cleanup = window.electronAPI.onSessionEvent((event: SessionEvent) => {
      ...
      if (event.type === 'info' && event.statusType === 'compaction_complete') {
        window.dispatchEvent(new CustomEvent('crabpal:compaction-complete', {
          detail: { sessionId }
        }))
      }
  ...
  const handleSendMessage = useCallback(async (sessionId: string, message: string, attachments?: FileAttachment[], skillSlugs?: string[], externalBadges?: ContentBadge[]) => {
    try {
      ...
      const slashCommand = extractSlashCommand(message)
      if (slashCommand === 'compact') {
        const commandText = `/${slashCommand}`
        badges.unshift(buildSlashCommandBadge(slashCommand, commandText))
      }
  ...
  // Platform actions for @crabpal/ui components (overlays, etc.)
