...
export function prepareSlackOAuth(options: PrepareSlackOAuthOptions): PreparedOAuthFlow {
  ...
  const redirectUri = options.callbackUrl
    ?? `https://crabpal.app/auth/slack/callback?port=${options.callbackPort}`;
...
export async function startSlackOAuth(options: SlackOAuthOptions = {}): Promise<SlackOAuthResult> {
  try {
    ...
    // The relay redirects: https://crabpal.app/auth/slack/callback → http://localhost:{port}/callback
    const redirectUri = `https://crabpal.app/auth/slack/callback?port=${port}`;
