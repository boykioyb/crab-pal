...
async function ensureSession(): Promise<AgentSession> {
  ...
  if (initConfig.sessionPath) {
    ...
    // Session resume: use a per-CrabPal-session directory so the Pi SDK can
 ...
 * crash on a shared AbortController (see crabpal-oss#464).
 */
...
async function handlePrompt(msg: Extract<InboundMessage, { type: 'prompt' }>): Promise<void> {
  ...
  try {
    ...
    // Wait for any in-flight auto-compaction to avoid race (crabpal-oss#464)
