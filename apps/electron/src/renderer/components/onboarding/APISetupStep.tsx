...
import type { LlmAuthType, LlmProviderType } from "@crabpal/shared/config/llm-connections"
...
const SEGMENT_DESCRIPTIONS: Record<ProviderSegment, React.ReactNode> = {
  ...
  pi: <>Use CrabPal Backend as the main agent.<BetaBadge /><br />Configure with your API key, OAuth subscription, or custom endpoint.</>,
}
...
const API_SETUP_OPTIONS: ApiSetupOption[] = [
  ...
  },
  {
    id: 'pi_chatgpt_oauth',
    name: 'ChatGPT Plus',
    description: 'Use your ChatGPT subscription with CrabPal Backend.',
  ...
  },
  {
    id: 'pi_copilot_oauth',
    name: 'GitHub Copilot',
    description: 'Use your GitHub Copilot subscription with CrabPal Backend.',
    icon: <Cpu className="size-4" />,
    providerType: 'pi',
  },
  {
    id: 'pi_api_key',
    name: 'API Key',
    description: 'Use provider presets (Anthropic, OpenAI, Google, etc.) via CrabPal Backend.',
    icon: <Key className="size-4" />,
    providerType: 'pi',
  },
]
...
function OptionButton({
  ...
  return (
    <button
      ...
      className={cn(
        "flex w-full items-start gap-4 rounded-lg p-4 text-left transition-all",
...
function ProviderSegmentedControl({
  ...
  return (
    <div className="flex rounded-lg bg-foreground/[0.03] p-1 mb-4">
