...
import { Icon_Folder, Spinner } from '@crabpal/ui'
...
import { Tooltip, TooltipContent, TooltipTrigger } from '@crabpal/ui'
import {
  DropdownMenu,
  DropdownMenuTrigger,
  DropdownMenuSub,
...
import { type ThinkingLevel, THINKING_LEVELS, getThinkingLevelName } from '@crabpal/shared/agent/thinking-levels'
...
import { projectByIdAtom } from '@/atoms/projects'
...
export interface FreeFormInputProps {
  ...
  workspaceId?: string
  /** Project ID the session belongs to (drives the Project badge) */
  projectId?: string
...
export function FreeFormInput({
  ...
  const connectionsByProvider = React.useMemo(() => {
    ...
    for (const conn of llmConnections) {
      ...
      if (provider === 'anthropic') {
        ...
        groups['CrabPal Backend'].push(conn)
      }
    }
  ...
  // Listen for crabpal:insert-text events (generic mechanism for inserting text into input)
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('crabpal:insert-text', handleInsertText as EventListener)
    return () => window.removeEventListener('crabpal:insert-text', handleInsertText as EventListener)
  ...
  // Listen for crabpal:approve-plan events (used by ResponseCard's Accept Plan button)
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('crabpal:approve-plan', handleApprovePlan as EventListener)
    return () => window.removeEventListener('crabpal:approve-plan', handleApprovePlan as EventListener)
  }, [sessionId, permissionMode, onPermissionModeChange, onSubmit, consumeInputDraftSnapshot])

  // Listen for crabpal:approve-plan-with-compact events (Accept & Compact option)
  ...
  React.useEffect(() => {
    const handleApprovePlanWithCompact = async (e: CustomEvent<PlanApprovalEventDetail>) => {
      ...
      const handleCompactionComplete = async (compactEvent: CustomEvent<{ sessionId?: string }>) => {
        ...
        window.removeEventListener('crabpal:compaction-complete', handleCompactionComplete as unknown as EventListener)
      ...
      window.addEventListener('crabpal:compaction-complete', handleCompactionComplete as unknown as EventListener)
    }

    window.addEventListener('crabpal:approve-plan-with-compact', handleApprovePlanWithCompact as unknown as EventListener)
    return () => window.removeEventListener('crabpal:approve-plan-with-compact', handleApprovePlanWithCompact as unknown as EventListener)
  }, [sessionId, permissionMode, onPermissionModeChange, onSubmit, consumeInputDraftSnapshot])
  ...
  // Listen for crabpal:focus-input events (restore focus after popover/dropdown closes)
  React.useEffect(() => {
    ...
    window.addEventListener('crabpal:focus-input', handleFocusInput)
    return () => window.removeEventListener('crabpal:focus-input', handleFocusInput)
  ...
  // Listen for crabpal:paste-files events (for global paste when input not focused)
  React.useEffect(() => {
    ...
    window.addEventListener('crabpal:paste-files', handlePasteFiles as unknown as EventListener)
    return () => window.removeEventListener('crabpal:paste-files', handlePasteFiles as unknown as EventListener)
  ...
  // Listen for crabpal:submit-input events (simulate pressing the Send button)
  React.useEffect(() => {
    ...
    window.addEventListener('crabpal:submit-input', handleSubmitInput as EventListener)
    return () => window.removeEventListener('crabpal:submit-input', handleSubmitInput as EventListener)
    ...
    <form onSubmit={handleSubmit}>
      <div
        ...
        className={cn(
          ...
          !unstyled && 'rounded-lg shadow-middle',
      ...
      >
        ...
        <RichTextInput
          ...
          onBlur={() => {
            // Save caret position before losing focus (for restoration via crabpal:focus-input)
        ...
        <div className="relative">
          ...
          <div className={cn("flex items-center gap-1 px-2 py-2", !compactMode && "border-t border-border/30")}>
          ...
          {/* 3. Project Badge (with optional working-directory override) */}
          {onWorkingDirectoryChange && (
            <ProjectBadge
              projectId={projectId}
              workingDirectory={workingDirectory}
          ...
          {/* Right side: Connection + Model + Send - never shrink so they're always visible */}
          <div className="flex items-center shrink-0 gap-1">
          {/* 4b. Connection Selector — always visible when multiple connections exist.
              Shows which account's token the current session is using so the user
              can confirm at a glance and switch without opening the model picker. */}
          {!compactMode && onConnectionChange && llmConnections.length > 1 && effectiveConnectionDetails && !connectionUnavailable && (
            <DropdownMenu open={connectionDropdownOpen} onOpenChange={setConnectionDropdownOpen}>
              <Tooltip>
                <TooltipTrigger asChild>
                  <DropdownMenuTrigger asChild>
                    ...
                    >
                      ...
                      <span className="truncate max-w-[120px]">{effectiveConnectionDetails.name}</span>
                      <ChevronDown className="h-3 w-3 opacity-50 shrink-0" />
                    </button>
                  </DropdownMenuTrigger>
                </TooltipTrigger>
                <TooltipContent side="top">Connection — next turn uses this account</TooltipContent>
              </Tooltip>
              <StyledDropdownMenuContent side="top" align="end" sideOffset={8} className="min-w-[240px]">
                {llmConnections.map((conn) => {
                  const isCurrent = effectiveConnection === conn.slug
                  return (
                    <StyledDropdownMenuItem
                      key={conn.slug}
                      disabled={!conn.isAuthenticated || isCurrent}
                      onSelect={() => {
                        if (!isCurrent && conn.isAuthenticated) {
                    ...
                    >
                      ...
                      {isCurrent && <Check className="h-3 w-3 text-foreground shrink-0 ml-3" />}
                    </StyledDropdownMenuItem>
                  )
                })}
              </StyledDropdownMenuContent>
                ...
                <DropdownMenuTrigger asChild>
                  ...
                  >
                    {connectionUnavailable ? (
                      ...
                      <>
                        {effectiveConnectionDetails && llmConnections.length > 1 && storage.get(storage.KEYS.showConnectionIcons, true) && (compactMode || !onConnectionChange || connectionUnavailable) && <ConnectionIcon connection={effectiveConnectionDetails} size={14} showTooltip />}
...
Model
            ...
            <StyledDropdownMenuContent side="top" align="end" sideOffset={8} className="min-w-[260px]">
              ...
              {connectionUnavailable ? (
                ...
                <>
                  {!isEmptySession && onConnectionChange && llmConnections.length > 1 && (
                    <>
                      <DropdownMenuSub>
                        <StyledDropdownMenuSubTrigger className="flex items-center justify-between px-2 py-2 rounded-lg">
                          <div className="text-left flex-1">
                            <div className="font-medium text-sm">Switch connection</div>
                            <div className="text-xs text-muted-foreground">Next turn starts on the selected account</div>
                          </div>
                        </StyledDropdownMenuSubTrigger>
                        <StyledDropdownMenuSubContent className="min-w-[240px]">
                          {llmConnections.map((conn) => {
                            const isCurrentConnection = currentConnection === conn.slug
                            return (
                              <StyledDropdownMenuItem
                                key={conn.slug}
                                disabled={!conn.isAuthenticated || isCurrentConnection}
                                onSelect={() => {
                                  if (!isCurrentConnection && conn.isAuthenticated) {
                                    onConnectionChange(conn.slug)
                                  }
                                }}
                                className="flex items-center justify-between px-2 py-2 rounded-lg"
...
/**
 * ProjectBadge — Shows the session's owning project with an optional working
 * directory override.
 *
 * Default state: read-only indicator `📁 {project.name}`. Opening the menu
 * lets the user override the session's working directory (uses the same
 * directory picker as before). When overridden, the chip shows
 * `📁 {project.name} → {override basename}` plus a "Reset to project default"
 * menu entry. Passing a path equal to `project.rootPath` through
 * `onWorkingDirectoryChange` clears the override in practice — the backend
 * doesn't expose a separate "clear override" IPC today, so resetting sets the
 * session's working directory back to the project root.
 */
function ProjectBadge({
  ...
  // Resolve effective working directory purely in-view (no backend round-trip).
  //
  // session.workingDirectory is the override when set; otherwise fall back to
  // project.rootPath. This mirrors getEffectiveWorkingDirectory(session,
  // project) from @crabpal/core so tooltip / git-branch reflect the actual
  // path the agent will run from.
  const effectiveDir = workingDirectory ?? project?.rootPath ?? ''

  React.useEffect(() => {
    if (effectiveDir) {
      window.electronAPI?.getGitBranch?.(effectiveDir).then((branch: string | null) => {
        setGitBranch(branch)
      })
    } else {
      setGitBranch(null)
  ...
  const handleFolderSelected = React.useCallback((selectedPath: string) => {
    addRecentWorkingDir(selectedPath, workspaceId)
  ...
  const handleReset = () => {
    if (project) {
      onWorkingDirectoryChange(project.rootPath)
    }
    setMenuOpen(false)
  }

  // Legacy / unassigned session: surface a disabled chip so the user isn't
  // silently missing the project affordance. Full "assign project" flow lives
  // in sidebar UI (Phase 4); wiring it in here is out of scope.
  if (!project) {
    return (
      <span className="shrink min-w-0 overflow-hidden">
        <FreeFormInputContextBadge
          ...
          showChevron={false}
          disabled
          tooltip="This session has no project assigned. Use the sidebar to assign one."
        />
      </span>
    )
  }

  // Override is active when the session has its own working directory AND it
  // differs from the project's root path. Anything matching rootPath is
  // treated as "default" for display purposes even if persisted.
  const isOverridden = !!workingDirectory && workingDirectory !== project.rootPath
  const overrideBasename = isOverridden
    ? (getPathBasename(workingDirectory) || 'Folder')
    : ''

  const label = isOverridden
    ? `${project.name} → ${overrideBasename}`
    : project.name
  ...
  return (
    ...
    <DropdownMenu open={menuOpen} onOpenChange={setMenuOpen}>
            ...
            tooltip={
              <span className="flex flex-col gap-0.5">
                <span className="font-medium">
                  {isOverridden ? 'Working directory (override)' : `Project: ${project.name}`}
                </span>
                <span className="text-xs opacity-70">{formatPathForDisplay(effectiveDir, homeDir)}</span>
                {gitBranch && <span className="text-xs opacity-70">on {gitBranch}</span>}
                {isOverridden && (
                  <span className="text-xs opacity-70">
                    project root: {formatPathForDisplay(project.rootPath, homeDir)}
                  </span>
                )}
              </span>
            }
      ...
      <StyledDropdownMenuContent side="top" align="start" sideOffset={8}>
        <StyledDropdownMenuItem onSelect={handleOverride}>
          {isOverridden ? 'Change override folder…' : 'Override working directory…'}
        </StyledDropdownMenuItem>
        {isOverridden && (
          <>
            <StyledDropdownMenuSeparator />
            <StyledDropdownMenuItem onSelect={handleReset}>
