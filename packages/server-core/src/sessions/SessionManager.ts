...
import type { ISessionManager, IBrowserPaneManager } from '@crabpal/server-core/handlers'
import { createScopedLogger, CONSOLE_LOGGER, type PlatformServices, type Logger } from '@crabpal/server-core/runtime'
...
import { type AgentEvent, setPermissionMode, hydratePreviousPermissionMode, getPermissionModeDiagnostics, type PermissionMode, unregisterSessionScopedToolCallbacks, mergeSessionScopedToolCallbacks, AbortReason, type AuthRequest, type AuthResult, type CredentialAuthRequest, type BrowserPaneFns, generateConversationSummary } from '@crabpal/shared/agent'
import {
  ...
  type PostInitResult,
} from '@crabpal/shared/agent/backend'
import { getLlmConnection, getLlmConnections, getDefaultLlmConnection, getDefaultThinkingLevel, getModelsForProviderType, resetManagedAnthropicAuthEnvVars } from '@crabpal/shared/config'
import { PrivilegedExecutionBroker } from '@crabpal/server-core/services'
import { isValidWorkingDirectory } from '../utils/path-validation'
import { InitGate } from '@crabpal/server-core/domain'
import {
  getWorkspaces,
...
import type { ActiveSessionInfo, SessionProcessingStatus } from '@crabpal/core/types'
import { listProjects, getProject, seedDefaultProjectIfNeeded } from '@crabpal/core/storage'
import { loadWorkspaceConfig } from '@crabpal/shared/workspaces'
import {
  ...
  type SessionHeader,
  pickSessionFields,
} from '@crabpal/shared/sessions'
import { loadWorkspaceSources, loadAllSources, getSourcesBySlugs, isSourceUsable, type LoadedSource, type McpServerConfig, getSourcesNeedingAuth, getSourceCredentialManager, getSourceServerBuilder, type SourceWithCredential, isApiOAuthProvider, SERVER_BUILD_ERRORS, TokenRefreshManager, createTokenGetter } from '@crabpal/shared/sources'
import { ConfigWatcher, type ConfigWatcherCallbacks } from '@crabpal/shared/config'
import { getValidClaudeOAuthToken } from '@crabpal/shared/auth'
import { resolveAuthEnvVars } from '@crabpal/shared/config'
import { toolMetadataStore, getLastApiError } from '@crabpal/shared/interceptor'
import { isParentTaskTool } from '@crabpal/shared/utils/toolNames'
import { getCredentialManager } from '@crabpal/shared/credentials'
import { CrabPalMcpClient, McpClientPool, McpPoolServer } from '@crabpal/shared/mcp'
import { type Session, type SessionEvent, type FileAttachment, type SendMessageOptions, type UnreadSummary, type RemoteSessionTransferPayload, type ImportRemoteSessionTransferResult, RPC_CHANNELS, generateMessageId } from '@crabpal/shared/protocol'
import { messageToStored, storedToMessage, type Message, type StoredAttachment, type ToolDisplayMeta } from '@crabpal/core/types'
import { formatPathsToRelative, formatToolInputPaths, perf, encodeIconToDataUrlAsync, getEmojiIcon, resetSummarizationClient, resolveToolIcon, readFileAttachment, selectSpreadMessages, normalizePath } from '@crabpal/shared/utils'
import { loadAllSkills, loadSkillBySlug, invalidateSkillsCache, type LoadedSkill } from '@crabpal/shared/skills'
import { invalidateContextFileCache } from '@crabpal/shared/prompts/system'
import { getToolIconsDir, getMiniModel } from '@crabpal/shared/config'
import { getDefaultSummarizationModel } from '@crabpal/shared/config/models'
import type { SummarizeCallback } from '@crabpal/shared/sources'
import { type ThinkingLevel, DEFAULT_THINKING_LEVEL, normalizeThinkingLevel } from '@crabpal/shared/agent/thinking-levels'
import { evaluateAutoLabels } from '@crabpal/shared/labels/auto'
import { listLabels, loadLabelConfig } from '@crabpal/shared/labels/storage'
import { extractLabelId } from '@crabpal/shared/labels'
import { ensureLabelsExist } from '@crabpal/shared/labels/crud'
import { flattenLabels } from '@crabpal/shared/labels/tree'
import { loadStatusConfig } from '@crabpal/shared/statuses/storage'
import { AutomationSystem, createPromptHistoryEntry, appendAutomationHistoryEntry, type AutomationSystemMetadataSnapshot } from '@crabpal/shared/automations'

// Import from server-core domain utilities
import { sanitizeForTitle, shouldActivateBrowserOverlay, normalizeBrowserToolName, rollbackFailedBranchCreation, releaseBrowserOwnershipOnForcedStop } from '@crabpal/server-core/domain'
import { resizeImageForAPI, resizeIconBuffer } from '@crabpal/server-core/services'
...
async function applyBridgeUpdates(
  ...
  mcpServers: Record<string, import('@crabpal/shared/agent/backend').SdkMcpServerConfig>,
  sessionId: string,
...
async function getBrowserToolIconDataUrl(): Promise<string | undefined> {
  ...
  try {
    const iconCandidates = [
      ...
      // Dev fallback (before sync to ~/.crabpal/tool-icons)
...
async function resolveToolDisplayMeta(
  ...
  // and resolves their brand icon from ~/.crabpal/tool-icons/
...
interface ManagedSession {
  id: string
  workspace: Workspace
  /** Project (inside the workspace) this session belongs to.
   *  Required for new sessions created via createSession(); may be absent on
   *  ManagedSessions constructed from legacy on-disk headers. */
  projectId?: string
...
export class SessionManager implements ISessionManager {
  ...
  private pendingCredentialResolvers: Map<string, (response: import('@crabpal/shared/protocol').CredentialResponse) => void> = new Map()
  ...
  setupConfigWatcher(workspaceRootPath: string, workspaceId: string): void {
    ...
    const callbacks: ConfigWatcherCallbacks = {
      ...
      onSkillChange: async (slug, skill) => {
        ...
        const { loadAllSkills } = await import('@crabpal/shared/skills')
        const skills = loadAllSkills(workspaceRootPath)
        this.broadcastSkillsChanged(workspaceId, skills)
      },
  ...
  private broadcastAppThemeChanged(theme: import('@crabpal/shared/config').ThemeOverrides | null): void {
    if (!this.eventSink) return
  ...
  private broadcastSkillsChanged(workspaceId: string, skills: import('@crabpal/shared/skills').LoadedSkill[]): void {
    if (!this.eventSink) return
  ...
  private async reloadSessionSources(managed: ManagedSession): Promise<void> {
    ...
    // Reload all sources from disk (crabpal-docs is always available as MCP server)
   ...
   * See: https://github.com/lukilabs/crabpal-oss/issues/39
   */
  /**
   * Reinitialize authentication environment variables.
   *
  ...
  /**
   * Resolve a projectId to assign when the caller didn't supply one
   * (automations, remote transfers, spawned sessions whose parent predates
   * the Project hierarchy). Seeds the workspace's Default project if needed
   * so this always returns a non-empty string.
   */
  private resolveDefaultProjectId(workspaceRootPath: string, workspaceId: string): string {
    // Defensive: ensure Default project exists (idempotent).
    try {
      seedDefaultProjectIfNeeded(workspaceRootPath, workspaceId)
    } catch (err) {
      sessionLog.warn(`resolveDefaultProjectId: seed failed for ${workspaceId}: ${err}`)
    }
    const projects = listProjects(workspaceRootPath)
    if (projects.length === 0) {
      throw new Error(
        `No projects found for workspace ${workspaceId}; cannot resolve a default project.`,
      )
    }
    // listProjects returns sorted by lastUsedAt desc — pick the most recent.
  ...
  async handleCredentialInput(
    sessionId: string,
    requestId: string,
    response: import('@crabpal/shared/protocol').CredentialResponse
  ): Promise<void> {
    const managed = this.sessions.get(sessionId)
    ...
    try {
      ...
      const { markSourceAuthenticated } = await import('@crabpal/shared/sources')
  ...
  async createSession(workspaceId: string, options?: import('@crabpal/shared/protocol').CreateSessionOptions): Promise<Session> {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) {
      throw new Error(`Workspace ${workspaceId} not found`)
    }

    // Phase 3: every session must be assigned to a project up-front.
    // Callers (renderer action atoms, automation pipelines, etc.) are expected
    // to resolve a projectId before invoking createSession. Silent fallback to
    // a "default" project would hide bugs, so we reject instead.
    const projectId = options?.projectId
    if (!projectId || typeof projectId !== 'string' || projectId.trim() === '') {
      throw new Error(
        'createSession: projectId is required. ' +
        'Every session must belong to a project inside the workspace.',
      )
    }
    ...
    // - 'user_default' or undefined: Use project.rootPath, falling back to
    //   the workspace-level configured default. Project.rootPath is the
    //   primary source per the Workspace → Project → Session hierarchy.
    // - 'none': No working directory (empty string means session folder only)
    // - Absolute path: Use as-is
    const projectForCwd = getProject(workspaceRootPath, projectId)
    const projectRootPath = projectForCwd?.rootPath
    ...
    if (options?.workingDirectory === 'none') {
      ...
      resolvedWorkingDir = projectRootPath ?? userDefaultWorkingDir
    } else {
  ...
  private async getOrCreateAgent(managed: ManagedSession): Promise<AgentInstance> {
    if (!managed.agent) {
      ...
      // The SDK subprocess reads CRAB_PAL_SESSION_DIR to write tool-metadata.json;
      ...
      process.env.CRAB_PAL_SESSION_DIR = sessionDirForMetadata
      ...
      const envOverrides: Record<string, string> = {
        CRAB_PAL_WORKSPACE_PATH: managed.workspace.rootPath,
      ...
      managed.agent = createBackendFromResolvedContext({
        ...
        enable1MContext: await (async () => { const { getEnable1MContext } = await import('@crabpal/shared/config/storage'); return getEnable1MContext(); })(),
      ...
      managed.agent.onSpawnSession = async (request) => {
        ...
        // Spawned sessions inherit the parent session's project assignment.
        // Falls back to the workspace's Default project if the parent lacks one
        // (e.g. legacy pre-Phase-3 sessions reloaded from disk).
        const spawnProjectId = managed.projectId
          ?? this.resolveDefaultProjectId(managed.workspace.rootPath, managed.workspace.id)
  ...
  async setSessionConnection(sessionId: string, connectionSlug: string): Promise<void> {
    ...
    if (managed.isProcessing) {
      ...
      throw new Error('Cannot change connection while the session is processing')
    }

    // Validate connection exists
    const { getLlmConnection } = await import('@crabpal/shared/config/storage')
    ...
    // Early-return only when the session is already LOCKED to this exact connection.
    // We must NOT early-return just because llmConnection happens to match — an earlier
    // setSessionModel() call may have set llmConnection without locking, and the renderer
    // never received a connection_changed event. Falling through lets us lock and fire
    // the event so the UI stays in sync.
    if (managed.llmConnection === connectionSlug && managed.connectionLocked) {
      sessionLog.info(`setSessionConnection: session ${sessionId} already locked to ${connectionSlug}`)
      return
    }

    const hasStarted = !!managed.messages?.length || !!managed.sdkSessionId || !!managed.agent
    managed.llmConnection = connectionSlug
    managed.connectionLocked = true

    // If the session already started, force the next turn to build a fresh backend
    // on the new connection rather than reusing the old provider session.
    if (hasStarted) {
      ...
      if (managed.poolServer) {
        await managed.poolServer.stop().catch((err) => {
          sessionLog.warn(`setSessionConnection: failed to stop pool server for ${sessionId}: ${err instanceof Error ? err.message : String(err)}`)
        })
        managed.poolServer = undefined
      }
      if (managed.mcpPool) {
        await managed.mcpPool.disconnectAll().catch((err) => {
          sessionLog.warn(`setSessionConnection: failed to disconnect MCP pool for ${sessionId}: ${err instanceof Error ? err.message : String(err)}`)
        })
        managed.mcpPool = undefined
      }

      // Preserve the user's explicit model only when the target connection can serve it.
      if (managed.model) {
        const availableModels = connection.models && connection.models.length > 0
          ? connection.models
          : getModelsForProviderType(connection.providerType, connection.piAuthProvider)
        const supportsCurrentModel = availableModels.some((candidate) =>
          (typeof candidate === 'string' ? candidate : candidate.id) === managed.model,
        )

        if (!supportsCurrentModel) {
          managed.model = undefined
          this.sendEvent({
   ...
   */
  async shareToViewer(sessionId: string): Promise<import('@crabpal/shared/protocol').ShareResult> {
    const managed = this.sessions.get(sessionId)
    if (!managed) {
    ...
    try {
      ...
      const { VIEWER_URL } = await import('@crabpal/shared/branding')
   ...
   */
  async updateShare(sessionId: string): Promise<import('@crabpal/shared/protocol').ShareResult> {
    const managed = this.sessions.get(sessionId)
    if (!managed) {
      return { success: false, error: 'Session not found' }
    }
   ...
   */
  async revokeShare(sessionId: string): Promise<import('@crabpal/shared/protocol').ShareResult> {
    const managed = this.sessions.get(sessionId)
    if (!managed) {
      return { success: false, error: 'Session not found' }
    }
  ...
  async updateSessionModel(sessionId: string, workspaceId: string, model: string | null, connection?: string): Promise<void> {
    ...
    if (managed) {
      ...
      // If we also updated the connection as part of the model change, the renderer
      // needs a connection_changed event to keep session.llmConnection in sync.
      // Without this, the dropdown checkmark/selection reverts to the resolver
      // fallback on next open.
      if (connection && updates.llmConnection) {
        this.sendEvent({
          type: 'connection_changed',
          sessionId,
  ...
  async sendMessage(sessionId: string, message: string, attachments?: FileAttachment[], storedAttachments?: StoredAttachment[], options?: SendMessageOptions, existingMessageId?: string, _isAuthRetry?: boolean): Promise<void> {
    ...
    try {
      ...
      // in the rawText, and canUseTool in crabpal.ts provides a fallback
  ...
  respondToPermission(
    ...
    allowed: boolean,
    alwaysAllow: boolean,
    options?: import('@crabpal/shared/protocol').PermissionResponseOptions,
  ): boolean {
    const managed = this.sessions.get(sessionId)
    if (managed?.agent) {
   ...
   */
  async respondToCredential(sessionId: string, requestId: string, response: import('@crabpal/shared/protocol').CredentialResponse): Promise<boolean> {
  ...
  async executePromptAutomation(
    ...
    // Automations have no natural project affinity — assign to the workspace's
    // Default project so Phase 3's required-projectId invariant holds.
    const automationProjectId = this.resolveDefaultProjectId(workspaceRootPath, workspaceId)
  ...
  private async generateRemoteTransferSummary(managed: ManagedSession): Promise<string | null> {
    ...
    const envOverrides: Record<string, string> = {
      CRAB_PAL_WORKSPACE_PATH: workspaceRootPath,
      ...(miniModel ? { ANTHROPIC_DEFAULT_HAIKU_MODEL: miniModel } : {}),
    }
  ...
  async importRemoteSessionTransfer(
    ...
    // Remote transfers don't carry project context — assign to the target
    // workspace's Default project.
    const targetWorkspace = getWorkspaceByNameOrId(workspaceId)
    if (!targetWorkspace) {
      throw new Error(`Workspace ${workspaceId} not found`)
    }
    const transferProjectId = this.resolveDefaultProjectId(targetWorkspace.rootPath, workspaceId)
    const session = await this.createSession(workspaceId, {
