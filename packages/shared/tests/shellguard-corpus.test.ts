...
const TEST_MODE_CONFIG = {
  ...
  readOnlyBashPatterns: [
    ...
    // crabpal CLI read-only
    { regex: /^crabpal\s+label\s+(list|get)\b/, source: '^crabpal\\s+label\\s+(list|get)\\b', comment: 'crabpal label read-only operations' },
    { regex: /^crabpal\s+source\s+(list|get|validate|test)\b/, source: '^crabpal\\s+source\\s+(list|get|validate|test)\\b', comment: 'crabpal source read-only operations' },
    { regex: /^crabpal\s+skill\s+(list|get|validate|where)\b/, source: '^crabpal\\s+skill\\s+(list|get|validate|where)\\b', comment: 'crabpal skill read-only operations' },
    { regex: /^crabpal\s+automation\s+(list|get|validate|history|last-executed|test|lint)\b/, source: '^crabpal\\s+automation\\s+(list|get|validate|history|last-executed|test|lint)\\b', comment: 'crabpal automation read-only operations' },
    { regex: /^crabpal\s*$/, source: '^crabpal\\s*$', comment: 'crabpal bare invocation' },
    { regex: /^crabpal\s+(label|source|skill|automation)\s+--help\b/, source: '^crabpal\\s+(label|source|skill|automation)\\s+--help\\b', comment: 'crabpal entity help flags' },
    { regex: /^crabpal\s+--(help|version|discover)\b/, source: '^crabpal\\s+--(help|version|discover)\\b', comment: 'crabpal global flags' },
...
// ============================================================
// Group 27: crabpal CLI allowlist
// ============================================================

describe('ShellGuard corpus: crabpal CLI allowlist', () => {
  const shouldAllow = [
    'crabpal',
    'crabpal --help',
    'crabpal --version',
    ...
    'crabpal skill --help',
    'crabpal skill list',
    'crabpal skill get commit-helper',
    'crabpal skill where commit-helper',
    'crabpal skill validate commit-helper',
    'crabpal automation list',
    'crabpal automation get abc123',
    'crabpal automation validate',
    'crabpal automation history abc123 --limit 5',
    'crabpal automation last-executed abc123',
    'crabpal automation test abc123 --match "x"',
    'crabpal automation lint',
  ];

  const shouldBlock = [
    'crabpal label create --name Bug',
    'crabpal label update bug --name "Bug Report"',
    'crabpal label delete bug',
    'crabpal label move bug --parent root',
    'crabpal label reorder --parent root a b c',
    'crabpal source create --name Linear --provider linear --type mcp',
    'crabpal source update linear --json "{\"enabled\":false}"',
    'crabpal source delete linear',
    'crabpal skill create --name "Review" --description "x"',
    'crabpal skill update review --json "{\"description\":\"y\"}"',
    'crabpal skill delete review',
    'crabpal automation create --event UserPromptSubmit --prompt "x"',
    'crabpal automation update abc123 --json "{\"enabled\":false}"',
    'crabpal automation delete abc123',
    'crabpal automation enable abc123',
    'crabpal automation disable abc123',
