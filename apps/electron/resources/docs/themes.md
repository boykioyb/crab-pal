...
This guide explains how to customize the visual theme of CrabPal.

## Overview

CrabPal uses a 6-color theme system with support for both app-level defaults and per-workspace overrides.
...
3. **Preset themes**: `~/.crabpal/themes/{name}.json` - Complete theme packages
4. **Theme overrides**: `~/.crabpal/theme.json` - Override specific colors (app-level)
...
```
~/.crabpal/workspaces/{id}/config.json
```
...
Create `~/.crabpal/theme.json` to override specific colors:
...
Preset themes are complete theme packages stored at `~/.crabpal/themes/`. Each preset is a JSON file with theme colors and metadata.
...
2. Save it to `~/.crabpal/themes/{name}.json`
...
1. Create `~/.crabpal/theme.json` for overrides or `~/.crabpal/themes/{name}.json` for a preset
...
- Check file is in correct location (`~/.crabpal/theme.json` for overrides, `~/.crabpal/themes/` for presets)
