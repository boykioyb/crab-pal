...
import { isLikelyUsageLimitExhausted } from '../agent/errors.ts';
...
export function parseValidationError(msg: string): string {
  ...
  if (lowerMsg.includes('429') || lowerMsg.includes('rate limit') || lowerMsg.includes('quota')) {
    if (isLikelyUsageLimitExhausted(msg)) {
      return 'This account has exhausted its current usage quota. Switch credentials or wait for the quota to reset.';
    }
