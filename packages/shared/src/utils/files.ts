...
function readClipboardMacOS(): FileAttachment[] {
  ...
  try {
    const scriptFile = join(tmpdir(), `crabpal-clipboard-files-${Date.now()}.js`);
    const jxaScript = `
ObjC.import('AppKit');
...
function readClipboardImageDataWindows(): FileAttachment | null {
  const tempFile = join(tmpdir(), `crabpal-clipboard-${Date.now()}.png`);
...
function readClipboardImageDataMacOS(): FileAttachment | null {
  ...
  try {
    const scriptFile = join(tmpdir(), `crabpal-clipboard-script-${Date.now()}.js`);
    const jxaScript = `
ObjC.import('AppKit');
ObjC.import('Foundation');
