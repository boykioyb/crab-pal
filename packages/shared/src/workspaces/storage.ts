 ...
 * Default location: ~/.crabpal/workspaces/
 */
...
/**
 * Get the default workspaces directory (~/.crabpal/workspaces/)
 */
 ...
 * @param baseDir - Parent directory where workspace folders live (e.g., ~/.crabpal/workspaces/)
 ...
 * Returns paths to valid workspaces found in ~/.crabpal/workspaces/
 */
 ...
 * Resolution order: ENV (CRAB_PAL_LOCAL_MCP_ENABLED) > workspace config > default (true)
 *
 * @param rootPath - Absolute path to workspace root folder
...
export function isLocalMcpEnabled(rootPath: string): boolean {
  ...
  const envValue = process.env.CRAB_PAL_LOCAL_MCP_ENABLED;
...
export function ensurePluginManifest(rootPath: string, workspaceName: string): void {
  ...
  const manifest = {
    name: `crabpal-workspace-${workspaceName.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`,
    version: '1.0.0',
  };
