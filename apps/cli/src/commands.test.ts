...
describe('parseArgs', () => {
  ...
  it('falls back to env vars for url and token', () => {
    const prevUrl = process.env.CRAB_PAL_SERVER_URL
    const prevToken = process.env.CRAB_PAL_SERVER_TOKEN
    const prevCa = process.env.CRAB_PAL_TLS_CA

    process.env.CRAB_PAL_SERVER_URL = 'ws://env-server:8080'
    process.env.CRAB_PAL_SERVER_TOKEN = 'env-token'
    process.env.CRAB_PAL_TLS_CA = '/env/ca.pem'
    ...
    try {
      ...
      if (prevUrl === undefined) delete process.env.CRAB_PAL_SERVER_URL
      else process.env.CRAB_PAL_SERVER_URL = prevUrl
      if (prevToken === undefined) delete process.env.CRAB_PAL_SERVER_TOKEN
      else process.env.CRAB_PAL_SERVER_TOKEN = prevToken
      if (prevCa === undefined) delete process.env.CRAB_PAL_TLS_CA
      else process.env.CRAB_PAL_TLS_CA = prevCa
    }
  })
  ...
  it('--source accumulates into array', () => {
    ...
    expect(args.sources).toEqual(['crabpal-kb', 'github'])
  })
