...
import {
  CLI_DOMAIN_POLICIES,
  CRAB_PAL_AGENTS_CLI_BASH_GUARD_SCOPE_ENTRIES,
  CRAB_PAL_AGENTS_CLI_WORKSPACE_SCOPE_ENTRIES,
  type CliDomainNamespace,
} from '../../config/cli-domains.ts';
import { FEATURE_FLAGS } from '../../feature-flags.ts';
import { AGENTS_PLUGIN_NAME } from '../../skills/types.ts';
import { GLOBAL_AGENT_SKILLS_DIRS, PROJECT_AGENT_SKILLS_DIRS } from '../../skills/storage.ts';
import {
...
export function qualifySkillName(
  ...
  // Extract the bare slug — strip any existing qualifier (e.g. "CrabPalWS:commit" → "commit")
...
function resolveSkillPlugin(
  ...
  // 1. Project: {workingDir}/{.agents,.claude,.cursor}/skills/{slug}/SKILL.md
  if (workingDirectory) {
    for (const rel of PROJECT_AGENT_SKILLS_DIRS) {
      if (existsSync(join(workingDirectory, rel, bareSlug, 'SKILL.md'))) {
  ...
  // 3. Global: ~/{.agents,.claude,.cursor}/skills/{slug}/SKILL.md
  for (const dir of GLOBAL_AGENT_SKILLS_DIRS) {
    if (existsSync(join(dir, bareSlug, 'SKILL.md'))) {
      return `${AGENTS_PLUGIN_NAME}:${bareSlug}`;
    }
  }
...
export function getConfigCliRedirect(
  ...
  if (filePath && LABELS_BLOCKED_FILE_TOOLS.has(toolName)) {
    ...
    if (relativePath) {
      const labelsScopeMatch = CRAB_PAL_AGENTS_CLI_WORKSPACE_SCOPE_ENTRIES.find(
...
/**
 * Block bash commands that operate on guarded config paths unless they use crabpal commands.
...
export function getConfigDomainBashRedirect(
  ...
  if (/^crabpal\s+(label|automation|source|skill)\b/.test(command)) {
    return null;
  }
  ...
  const bashGuardEntries: Array<{ namespace: CliDomainNamespace; scope: string }> = CRAB_PAL_AGENTS_CLI_BASH_GUARD_SCOPE_ENTRIES
...
const BUILT_IN_MCP_SERVERS = new Set(['session', 'crabpal-docs']);
