        ...
        >
          {workspaceRootPath && (
            <EditPopover
              align="center"
              trigger={
                <button className="inline-flex items-center h-7 px-3 text-xs font-medium rounded-lg bg-background/80 border border-border/30 hover:bg-foreground/[0.05] transition-colors duration-150">
                  Add Source
                </button>
              }
              {...getEditConfig(
        ...
        return {
          ...
          menu: (
            <SourceMenu
              ...
              onOpenInNewWindow={() => window.electronAPI.openUrl(`crabpal://sources/source/${source.config.slug}?window=focused`)}
