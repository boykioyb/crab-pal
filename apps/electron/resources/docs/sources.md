...
This guide explains how to configure sources (MCP servers, APIs, local filesystems) in CrabPal.

> **CLI-first workflow (recommended):** Use `crabpal source ...` commands instead of editing source config files directly.
> - `crabpal source --help`
...
**Before doing anything else**, search for a specialized guide using the built-in docs MCP server (`crabpal-docs`):

```
mcp__crabpal-docs__SearchCrabPal({ query: "{service} source setup" })
```

**Available guides:** GitHub, Linear, Slack, Gmail, Google Calendar, Google Drive, Google Docs, Google Sheets, Outlook, Microsoft Calendar, Teams, SharePoint, Filesystem, Brave Search, Memory
...
- "To find issues in the Mobile iOS project: ..."
...
Agent: [FIRST: Searches for Linear guide]
       mcp__crabpal-docs__SearchCrabPal({ query: "linear source setup" })
...
User: Issue tracking for my iOS team, project called "Mobile iOS"
...
Agent: [Creates config.json with:
        ...
        - tagline: "Issue tracking for the Mobile iOS team"
        - OAuth authentication]
Agent: [Writes guide.md mentioning "Mobile iOS" project and iOS team context]
...
Agent: Linear is now set up! I've configured it to focus on the "Mobile iOS" project.
You can use it to:
- Search and view issues in Mobile iOS
...
- `~/.crabpal/workspaces/{workspaceId}/sources/{sourceSlug}/`
...
1. Create the source folder:
   ```bash
   mkdir -p ~/.crabpal/workspaces/{ws}/sources/my-source
   ```
