 ...
 * Includes only relevant items for the CrabPal app.
 *
 ...
 *
 * On Windows/Linux: Menu is hidden - all functionality is in the CrabPal logo menu.
...
export async function rebuildMenu(): Promise<void> {
  ...
  // Users access menu via the CrabPal logo dropdown in the app
  if (!isMac) {
  ...
  const template: Electron.MenuItemConstructorOptions[] = [
    ...
    ...(isMac ? [{
      label: 'CrabPal',
      submenu: [
        { role: 'about' as const, label: 'About CrabPal' },
        updateMenuItem,
        ...
        { type: 'separator' as const },
        { role: 'hide' as const, label: 'Hide CrabPal' },
        { role: 'hideOthers' as const },
        { role: 'unhide' as const },
        { type: 'separator' as const },
        { role: 'quit' as const, label: 'Quit CrabPal' }
      ]
    }] : []),
