...
export function buildClaudeSubprocessEnv(
    ...
    // per-session via envOverrides in postInit(). Clearing them here first
    // ensures envOverrides has full authority: sessions using different
    // connections (OAuth vs API key vs Bedrock) cannot leak tokens into each other.
    delete env.ANTHROPIC_API_KEY;
    delete env.CLAUDE_CODE_OAUTH_TOKEN;
    delete env.ANTHROPIC_BASE_URL;
    delete env.ANTHROPIC_DEFAULT_HAIKU_MODEL;

    // Bedrock routing vars must never reach the Claude SDK subprocess path.
    // Strip only Claude-specific vars; keep generic AWS_* so user tooling works.
    delete env.CLAUDE_CODE_USE_BEDROCK;
    delete env.AWS_BEARER_TOKEN_BEDROCK;
    delete env.ANTHROPIC_BEDROCK_BASE_URL;

    // Apply per-session overrides last so they win over everything above.
    if (envOverrides) {
