...
export function PanelHeader({
  ...
  const titleContent = (
    ...
    >
      <h1 className={cn(
        "truncate font-sans text-[13px] font-semibold leading-tight tracking-[-0.015em] text-foreground",
          ...
          {titleMenu ? (
            <DropdownMenu open={dropdownOpen} onOpenChange={setDropdownOpen}>
              ...
              <button
                ...
                className={cn(
                  "titlebar-no-drag flex min-w-0 items-center gap-1.5 rounded-md border border-transparent px-3 py-1.5",
                  "transition-all duration-150 hover:border-foreground/8 hover:bg-foreground/[0.04]",
                  "focus:outline-none focus-visible:ring-1 focus-visible:ring-ring/50",
                  dropdownOpen && "border-foreground/10 bg-foreground/[0.06]"
                )}
              >
                {titleContent}
  ...
  const baseClassName = cn(
    'relative z-panel flex h-[48px] shrink-0 items-center gap-2.5 border-b border-border/30 bg-background/95 backdrop-blur-sm pr-3 min-w-0',
