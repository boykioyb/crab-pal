 ...
 */
import { getAuthState, getSetupNeeds } from '@crabpal/shared/auth'
import { getCredentialManager } from '@crabpal/shared/credentials'
import { setSetupDeferred } from '@crabpal/shared/config'
import { prepareClaudeOAuth, exchangeClaudeCode, fetchClaudeProfile, hasValidOAuthState, clearOAuthState, prepareMcpOAuth } from '@crabpal/shared/auth'
import { updateLlmConnection } from '@crabpal/shared/config/storage'
import { validateMcpConnection } from '@crabpal/shared/mcp'
...
export function registerOnboardingHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(RPC_CHANNELS.onboarding.EXCHANGE_CLAUDE_CODE, async (_ctx, authorizationCode: string, connectionSlug: string) => {
    try {
      ...
      // Fetch profile and cache it on the connection (best-effort — non-fatal if it fails).
      // This populates email/org/plan for the Account & Usage modal.
      fetchClaudeProfile(tokens.accessToken).then((profile) => {
        if (profile) {
          updateLlmConnection(connectionSlug, { profile })
          log.info(`[Onboarding] Claude profile fetched for ${connectionSlug}: ${profile.email ?? '(no email)'}`)
        }
      }).catch((err) => {
        log.warn('[Onboarding] Failed to fetch Claude profile:', err)
      })
