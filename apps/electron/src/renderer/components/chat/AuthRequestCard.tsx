...
import type { AuthRequestType, AuthStatus } from '@crabpal/core/types'
...
const VARIANT_STYLES: Record<AuthCardVariant, { bg: string; textClass: string; shadowColor?: string }> = {
  default: { bg: 'var(--background)', textClass: 'text-foreground shadow-sm' },
  ...
  muted: { bg: 'var(--foreground-3)', textClass: 'text-foreground/70 shadow-sm' },
}
...
export function AuthRequestCard({ message, onRespondToCredential, sessionId, isInteractive = true }: AuthRequestCardProps) {
  ...
  if (!isInteractive && authStatus !== 'pending') {
    ...
    return (
      <div
        className={cn('rounded-lg overflow-hidden w-fit select-none border border-border/30', variantTextClass)}
        style={{
  ...
  return (
    <div
      className={cn('rounded-lg overflow-hidden border border-border/30', variantTextClass)}
      style={{
        backgroundColor: variantBg,
        ...(shadowColor ? { '--shadow-color': shadowColor } as React.CSSProperties : {})
      }}
