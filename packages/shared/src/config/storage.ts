import { existsSync, mkdirSync, readFileSync, writeFileSync, rmSync, statSync, copyFileSync, readdirSync } from 'fs';
...
import type { StoredAttachment, StoredMessage } from '@crabpal/core/types';
...
import type { Workspace, AuthType } from '@crabpal/core/types';
...
// Legacy import helpers live in legacy-import.ts; re-exported for back-compat.
export {
  LEGACY_CONFIG_DIR,
  LEGACY_WORKSPACES_DIR,
  LEGACY_IMPORT_MARKER_FILE,
  LEGACY_IMPORT_FILES,
  LEGACY_IMPORT_DIRECTORIES,
  rewriteLegacyPathsInJson,
  copyDirectoryRecursive,
  importLegacyConfigDirOnce,
} from './legacy-import.ts';
import { importLegacyConfigDirOnce } from './legacy-import.ts';
...
const FALLBACK_CONFIG_DEFAULTS: ConfigDefaults = {
  version: '1.0',
  description: 'Default configuration values for CrabPal',
  defaults: {
...
/**
 * Load config defaults from ~/.crabpal/config-defaults.json
...
export function ensureConfigDir(): void {
  ...
  // Initialize bundled docs (creates ~/.crabpal/docs/ with sources.md, agents.md, permissions.md)
  initializeDocs();
 ...
 * causes 400 "Invalid Request" for lower-tier API keys on large contexts (upstream #567).
...
export type { StoredAttachment, StoredMessage } from '@crabpal/core/types';
...
/**
 * Get the path to the app-level theme override file (~/.crabpal/theme.json).
 */
 ...
 * Preset themes are stored at ~/.crabpal/themes/
 */
...
/**
 * Returns the path to the tool-icons directory: ~/.crabpal/tool-icons/
 */
