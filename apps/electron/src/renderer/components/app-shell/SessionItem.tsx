...
export function SessionItem({
  ...
  const previewText = getSessionPreviewText(item)
    ...
    <EntityRow
      ...
      menuContent={
        <SessionMenu
          ...
          onSelectGroup={() => ctx.onSelectGroupForItem(item.id)}
      ...
      icon={
        <>
          <div className="relative w-3 h-3 shrink-0">
            {/* Status icon - hidden on hover / when multi-select active */}
            <div className={cn(
              "absolute inset-0 flex items-center justify-center transition-opacity duration-100",
              ctx.isMultiSelectActive ? "opacity-0" : "opacity-100 group-hover:opacity-0"
            )}>
              <SessionStatusIcon item={item} />
            </div>
            {/* Selection checkbox - visible on hover or when multi-select is active */}
            <div
              role="checkbox"
              aria-checked={isInMultiSelect}
              aria-label={isInMultiSelect ? "Deselect session" : "Select session"}
              onMouseDown={(e) => {
                e.preventDefault()
                e.stopPropagation()
                onToggleSelect?.()
              ...
              className={cn(
                "absolute inset-0 flex items-center justify-center cursor-pointer transition-opacity duration-100",
            ...
            >
              <div className={cn(
                ...
                isInMultiSelect
                  ...
                  : "border-muted-foreground/50 hover:border-muted-foreground bg-background/60"
              )}>
                {isInMultiSelect && <Check className="!h-2 !w-2 text-accent-foreground" strokeWidth={3} />}
              </div>
            </div>
          </div>
          <div className={cn(
      ...
      titleTrailing={hasMatch ? (
        <span
          className={cn(
            "inline-flex min-w-[26px] items-center justify-center rounded-[8px] px-1.5 py-1 text-[10px] font-semibold leading-tight tabular-nums whitespace-nowrap shadow-tinted",
            isSelected
              ? "border border-yellow-500 bg-yellow-300/55 text-yellow-950"
              : "border border-yellow-600/20 bg-yellow-300/10 text-yellow-900"
          )}
          style={{
        ...
        <span className="whitespace-nowrap text-[11px] font-medium text-foreground/35">
