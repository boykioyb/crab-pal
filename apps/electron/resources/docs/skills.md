...
This guide explains how to create and configure skills in CrabPal.

> **CLI-first workflow (recommended):** Use `crabpal skill ...` commands instead of editing `SKILL.md` files directly.
> - `crabpal skill --help`
...
CrabPal uses **the identical SKILL.md format** as the Claude Code SDK. This means:

1. **Format compatibility**: Any skill written for Claude Code works in CrabPal
...
1. **Workspace skill checked first** - If `~/.crabpal/workspaces/{id}/skills/commit/SKILL.md` exists, it's used
...
```
~/.crabpal/workspaces/{workspaceId}/skills/{slug}/
...
```bash
mkdir -p ~/.crabpal/workspaces/{ws}/skills/my-skill
```
...
1. Create `~/.crabpal/workspaces/{ws}/skills/commit/SKILL.md`
