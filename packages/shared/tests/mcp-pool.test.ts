...
describe('McpClientPool.sync — config change detection', () => {
  ...
  it('reconnects when Authorization header changes (token refresh)', async () => {
    await pool.sync({ crabpal: httpConfig('old-token') });
    expect(pool.isConnected('crabpal')).toBe(true);
    pool.resetTracking();

    await pool.sync({ crabpal: httpConfig('new-token') });

    expect(pool.disconnectCalls).toEqual(['crabpal']);
  ...
  it('does not reconnect when config is unchanged', async () => {
    ...
    await pool.sync({ crabpal: config });
    pool.resetTracking();
  ...
  it('reconnects when URL changes', async () => {
    await pool.sync({ crabpal: httpConfig('token', 'https://old.example.com') });
    pool.resetTracking();

    await pool.sync({ crabpal: httpConfig('token', 'https://new.example.com') });
  ...
  it('does not reconnect when only non-auth headers change', async () => {
    ...
    await pool.sync({ crabpal: config1 });
    pool.resetTracking();

    await pool.sync({ crabpal: config2 });
  ...
  it('disconnects sources removed from config', async () => {
    ...
    await pool.sync({ crabpal: config, linear: config });
    pool.resetTracking();
  ...
  it('handles add + remove + refresh in a single sync', async () => {
    await pool.sync({
      crabpal: httpConfig('old-crabpal-token'),
      linear: httpConfig('linear-token', 'https://linear.example.com'),
    });
    pool.resetTracking();

    // crabpal: token refreshed, linear: removed, github: added
    await pool.sync({
      crabpal: httpConfig('new-crabpal-token'),
      github: httpConfig('gh-token', 'https://github.example.com'),
    });

    expect(pool.disconnectCalls).toContain('linear');
    expect(pool.disconnectCalls).toContain('crabpal');
    expect(pool.connectCalls.find(c => c.slug === 'crabpal')?.config.headers?.Authorization).toBe('Bearer new-crabpal-token');
  ...
  it('reports failure when reconnect fails after token refresh', async () => {
    ...
    await failPool.sync({ crabpal: httpConfig('old-token') });
    expect(failPool.isConnected('crabpal')).toBe(true);

    // Token refresh — disconnect succeeds but reconnect throws
    const failures = await failPool.sync({ crabpal: httpConfig('new-token') });

    expect(failures).toContain('crabpal');
    expect(failPool.isConnected('crabpal')).toBe(false);
  });
});
